...
def selftest() -> dict[str, Any]:
    ...
    try:
        引擎 = CNSH_流场可视化引擎()
        results["tests"]["引擎初始化"] = {"pass": True, "msg": f"根目录={引擎.根目录}"}
    except Exception as e:
        results["tests"]["引擎初始化"] = {"pass": False, "msg": str(e)}
    try:
        引擎 = CNSH_流场可视化引擎()
        数量 = 引擎.扫描文件粒子()
        results["tests"]["扫描粒子"] = {"pass": 数量 >= 0, "msg": f"扫描到{数量}个粒子"}
    except Exception as e:
        results["tests"]["扫描粒子"] = {"pass": False, "msg": str(e)}
    try:
        引擎 = CNSH_流场可视化引擎()
        健康 = 流场健康度(引擎)
        score = 健康.计算()
        results["tests"]["健康度"] = {"pass": "总分" in score, "msg": f"总分={score.get('总分', 'N/A')}"}
    except Exception as e:
        results["tests"]["健康度"] = {"pass": False, "msg": str(e)}
