...
DNA:#龍芯⚡️丙午·甲午·癸丑·戊午·䷨损-FORMULA-CHAIN-FILE5-v2.0-OPTIMIZED
...
def cnsh_package(trace: Dict[str, Any], dna: str = "") -> Dict[str, Any]:
    ...
    M::   → 机器可读：状态、决策、关键数值
    CNSH:: → 主权视角：DNA、确认码、封印、三色、五行、策略
    """
    color = trace.get("color", "⚪")
    decision = trace.get("决策", "UNKNOWN")
    element = trace.get("五行", "")
    score = trace.get("综合分", 0.0)
    risk = trace.get("加权风险", 0.0)
    action = trace.get("行动", "")
    si = trace.get("三才SI", {})
    ...
    # policy 映射
    policy_map = {"PASS": "pass", "REVIEW": "hold", "REJECT": "reject"}
    policy = policy_map.get(decision, "error")

    if not dna:
        h = hashlib.sha256(f"{trace['输入']}{decision}{time.time()}".encode()).hexdigest()[:8].upper()
        dna = f"#龍芯⚡️formula-chain-{h}"
    ...
    return {
        "M::": {
            ...
            "payload": {
                "input": trace.get("输入"),
                "digital_root": trace.get("数字根"),
                "wuxing": element,
                "color": color,
                "SI": si.get("SI") if isinstance(si, dict) else None,
                "risk": risk,
                "score": score,
                "decision": decision,
                "action": action,
        ...
        "CNSH::": {
            ...
            "wuxing": element,
            "policy": policy,
            "trace_hash": hashlib.sha256(json.dumps(trace, ensure_ascii=False, default=str).encode()).hexdigest()[:16],
        }
    }


def decision_chain_cnsh(
    n: int,
    risk_factors: List[float],
    weights: List[float],
    tian: float = 0.85,
    ...
    """
    trace = decision_chain(n, risk_factors, weights, tian, di, ren, si_weights, score_thresholds)
    return cnsh_package(trace, dna)
...
def selftest() -> None:
    ...
    # 9. CNSH 双视角封装（M:: 在前 · CNSH:: 在后）
    pkg = decision_chain_cnsh(
        20260603, [0.05, 0.05], [1, 1],
        tian=0.9, di=0.9, ren=0.9,
        dna="#龍芯⚡️丙午·甲午·甲戌·庚午·䷕贲-FORMULA-CHAIN-CNSH-TEST"
    )
    assert "M::" in pkg and "CNSH::" in pkg
    assert pkg["M::"]["status"] == "pass"
    assert pkg["CNSH::"]["audit"] == "🟢"
    assert pkg["CNSH::"]["gate"] == CONFIRM_CODE
    assert pkg["CNSH::"]["seal"] == SEAL_CODE
    print(f"[9] CNSH 双视角封装·M::+CNSH:: 完整·audit={pkg['CNSH::']['audit']}  ✅")
