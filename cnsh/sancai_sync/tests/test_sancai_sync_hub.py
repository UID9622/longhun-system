...
DNA:#龍芯⚡️2026-06-06-SANCAI-SYNC-TEST-SUITE-FILE1-v1.0
...
class TestDataStructures:
    ...
    def test_palace_node_creation(self):
        ...
        assert palace.palace_name == "干宫"
        assert palace.element == "金"


class TestSancaiSyncHub:
    """测试 SancaiSyncHub 核心类"""
    ...
    def test_ipa_to_particle_conversion(self, hub, sample_ipa):
        ...
        assert all(p.life > 0 for p in particles)  # pass 信号应有正生命值
    ...
    def test_ipa_to_particle_fuse_signal(self, hub, sample_ipa):
        ...
        assert all(p.life == 0 for p in particles)  # fuse 信号应导致零生命值
    ...
    def test_knowledge_to_palace_conversion(self, hub, sample_knowledge_graph):
        ...
        assert len(palaces) == 3  # 三个节点→三个宫位
...
class TestEdgeCases:
    ...
    def test_extreme_ring_values(self):
        ...
        assert len(signals) > 0
        assert all(s.activation >= 0.1 for s in signals)  # activation 不应低于 0.1
