...
class LonghunTrustScorer:
    ...
    def score(self, materials: dict[str, Any]) -> float:
...
class LonghunPrivacyCircuitBreaker:
    ...
    def inject(self, R: float, now: float | None = None) -> dict[str, Any]:
        if now is None:
            now = time.time()
    ...
    def request_recover(self, now: float | None = None, human_signature: bool = False) -> dict[str, Any]:
        if now is None:
            now = time.time()
...
class LonghunPrivacyAccessController:
    ...
    # ---------- 5.10 黑名单匹配 ----------
    def _check_blacklist(self, entity: dict[str, Any]) -> tuple[bool, bool]:
        """返回 (是否精确命中, 是否疑似换皮)。"""
    ...
    # ---------- 5.1 技术审计 ----------
    def _technical_audit(self, entity: dict[str, Any]) -> bool:
        required = [
            "code_audit_report",
            "encryption_scheme",
    ...
    # ---------- 5.1 地理合规（四信号投票） ----------
    def _geo_compliance(self, entity: dict[str, Any]) -> tuple[bool, int]:
    ...
    # ---------- 隐私规则执行（8.3） ----------
    def enforce_privacy_rules(self, request: dict[str, Any]) -> dict[str, Any]:
        """
        对单次数据请求执行隐私规则。
        request: {
...
# 第十八章：测试向量
# ═══════════════════════════════════════════════════════════
def run_test_vectors() -> dict[str, Any]:
    """执行 14 条测试向量，返回统计结果。"""
