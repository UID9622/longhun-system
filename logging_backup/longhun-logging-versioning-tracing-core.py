...
DNA:#龍芯⚡️2026-06-07-LOGGING-VERSIONING-TRACING-FILE1-v1.0
...
class LonghunLogger:
    ...
    def init_database(self) -> None:
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_compressed ON logs(compressed)")
        ...
        # 创建版本表索引
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_version ON versions(version)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_change_type ON versions(change_type)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_version_category ON versions(category)")
        ...
        # 创建压缩日志表索引
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_compressed_log_id ON compressed_logs(original_log_id)")
