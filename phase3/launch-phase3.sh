...
main() {
    ...
    print_step "第 1 步: 检查系统环境"
    check_environment

    print_step "第 2 步: 创建项目结构"
    create_project_structure

    print_step "第 3 步: 生成配置文件"
    generate_configs

    print_step "第 4 步: 构建 Docker 镜像"
    build_docker_images

    print_step "第 5 步: 启动服务"
    start_services

    print_step "第 6 步: 验证部署"
    verify_deployment

    print_step "第 7 步: 显示访问信息"
    show_access_info

    print_success "🎉 Phase 3 启动完成！"
}
...
build_docker_images() {
    ...
    print_step "构建 Docker 镜像（这可能需要 2-3 分钟）..."
...
verify_deployment() {
    ...
    if curl -s http://localhost:8000/api/v1/health > /dev/null 2>&1; then
        print_success "✓ 后端 API 正常运行"
        ((checks_passed++))
    else
        print_error "✗ 后端 API 无法连接"
    fi
    ...
    if [ $checks_passed -lt 2 ]; then
        print_warning "⚠️  某些服务可能未完全启动，请稍候片刻后重试"
        echo "查看日志: docker-compose -f longhun-phase3/docker-compose.yml logs"
    fi
}
...
show_access_info() {
    ...
    echo -e "  查看日志:      ${BLUE}docker-compose -f longhun-phase3/docker-compose.yml logs -f${NC}"
    echo -e "  停止服务:      ${BLUE}docker-compose -f longhun-phase3/docker-compose.yml down${NC}"
    echo -e "  重启服务:      ${BLUE}docker-compose -f longhun-phase3/docker-compose.yml restart${NC}"
    echo -e "  进入后端:      ${BLUE}docker-compose -f longhun-phase3/docker-compose.yml exec backend bash${NC}"
    echo ""
    echo -e "${YELLOW}📊 首次使用:${NC}"
    echo -e "  1. 访问 http://localhost:3000"
    echo -e "  2. 在“技能管理”页面创建新技能"
    echo -e "  3. 在“仪表板”查看实时监控"
    echo -e "  4. 在“告警系统”管理告警"
    echo ""
    echo -e "${YELLOW}🐉 DNA签章:${NC}"
    echo -e "  ${PURPLE}#龍芯⚡️2026-06-06-PHASE3-LAUNCH-SCRIPT-v1.0${NC}"
    echo -e "  ${PURPLE}责任: UID9622 · 不免责${NC}"
    echo ""
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
}
