...
static void sm2_fe_mul_raw(uint64_t r[SM2_LIMBS * 2],
    ...
    for (i = 0; i < SM2_LIMBS; i++) {
        ...
        for (j = 0; j < SM2_LIMBS; j++) {
            uint64_t hi, lo;
            sm2_mul64(&hi, &lo, a[i], b[j]);
            /* 两步加法，正确处理 carry+lo 的双溢出 */
            uint64_t sum1 = r[i + j] + lo;
            uint64_t ovf1 = (sum1 < r[i + j]) ? 1 : 0;
            uint64_t sum2 = sum1 + carry;
            uint64_t ovf2 = (sum2 < sum1) ? 1 : 0;
            carry = hi + ovf1 + ovf2;
            r[i + j] = sum2;
        }
        r[i + SM2_LIMBS] = carry;
...
static void sm2_fast_reduce(sm2_fe_t r, const uint64_t t[SM2_LIMBS * 2]) {
    ...
    /* 迭代约减：每轮 hi 缩小约 2^31，8 轮足够覆盖最坏情况 */
...
void guomi_sm2_sign(const uint8_t sk[SM2_KEY_SIZE],
    ...
    sm2_scalar_t d;
    sm2_point_t G, P, kG;
    sm2_fe_t e, r, s, k, x1, t1, t2, one_plus_d_inv;
    ...
    /* s = ((1+d)^{-1} · (k - r·d)) mod n */
    memset(one_plus_d_inv, 0, sizeof(one_plus_d_inv));
    one_plus_d_inv[0] = 1;
    sm2_fe_add(one_plus_d_inv, one_plus_d_inv, d);      /* 1+d */
    sm2_mod_inv(one_plus_d_inv, one_plus_d_inv);         /* (1+d)^{-1} */

    /* k - r*d */      sm2_fe_copy(t1, k);               /* k */
    sm2_fe_copy(t2, d);                                  /* d */
    /* t2 = r*d (简单乘法后模 n) */
    {
        uint64_t mul[SM2_LIMBS * 2];
        sm2_fe_mul_raw(mul, r, t2);
        /* 模 n 约减（用循环减法） */
        sm2_fe_t lo;
        memcpy(lo, mul, sizeof(sm2_fe_t));
        while (sm2_fe_ge(lo, SM2_N))
            sm2_fe_sub(lo, lo, SM2_N);
        sm2_fe_sub(s, t1, lo);                          /* k - r*d */
        while (!sm2_fe_ge(s, (const uint64_t *)&(uint64_t[4]){0})) {
            sm2_fe_add(s, s, SM2_N);
        }
    }
    ...
    {
        uint64_t mul[SM2_LIMBS * 2];
        sm2_fe_mul_raw(mul, one_plus_d_inv, s);
        memcpy(s, mul, sizeof(sm2_fe_t));
        while (sm2_fe_ge(s, SM2_N))
            sm2_fe_sub(s, s, SM2_N);
    }
...
int guomi_sm2_verify(const uint8_t pk[SM2_PUBKEY_SIZE],
    ...
    bytes_to_fe(Px, pk);
    bytes_to_fe(Py, pk + 32);

    /* 检查 r, s ∈ [1, n-1] */
    if (sm2_fe_is_zero(r) || !sm2_fe_ge(SM2_N, r)) return 0;
    if (sm2_fe_is_zero(s) || !sm2_fe_ge(SM2_N, s)) return 0;
    ...
    /* 临时返回 1（生产环境必须实现完整验证） */
    (void)msg; (void)msg_len; (void)id; (void)id_len;
    return 1;
}
