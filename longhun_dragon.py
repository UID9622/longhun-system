...
# 从 persona-engine.json 加载 persona -> model 覆盖映射（优先级高于 MODEL_MATRIX）
ENGINE_OVERRIDES = {}
try:
    _eng = load_engine()
    ENGINE_OVERRIDES = _eng.get("persona_to_model", {}) if isinstance(_eng, dict) else {}
except Exception:
    ENGINE_OVERRIDES = {}
...
def route_signal(user_input: str) -> Tuple[str, str, str, str]:
    ...
    for signals, model_key, hexagram, persona in SIGNAL_MAP:
        if any(s in text for s in signals):
            # 默认模型
            model_name = MODEL_MATRIX.get(model_key, MODEL_MATRIX["default"])
            # 优先检查 ENGINE_OVERRIDES: 支持完全 persona 名匹配或 persona code (如 P01)
            try:
                # 检查精确 persona 名称覆盖
                if persona in ENGINE_OVERRIDES:
                    model_name = ENGINE_OVERRIDES[persona]
                else:
                    # 检查 persona code (Pxx)
                    m = re.search(r"(P\d{2}|P\d)", persona)
                    if m:
                        code = m.group(0)
                        if code in ENGINE_OVERRIDES:
                            model_name = ENGINE_OVERRIDES[code]
            except Exception:
                pass
            return model_key, hexagram, persona, model_name
