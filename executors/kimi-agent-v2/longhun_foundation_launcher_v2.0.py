...
═══════════════════════════════════════════════════════════════════════════════
  ...
  DNA簽名    :#龍芯⚡️2026-06-17-FOUNDATION-LAUNCHER-FILE3-v2.0
...
用法:
  python longhun_foundation_launcher_v2.0.py        # 交互式啟動台
  python longhun_foundation_launcher_v2.0.py --auto  # 自動系統檢查
"""
...
class SystemHealthChecker:
    ...
    def check_disk_space(self, path: str = str(_PROJECT_ROOT / "var" / "kimi-agent-v2"), min_free_gb: float = 1.0) -> Dict[str, Any]:
        """檢查磁盤空間"""
        try:
    ...
    def check_log_system(self, log_dir: str = str(LOGS_DIR)) -> Dict[str, Any]:
        """檢查日誌系統"""
        path = Path(log_dir)
        if not path.exists():
            try:
    ...
    def check_checkpoint_system(self, checkpoint_dir: str = str(CHECKPOINTS_DIR)) -> Dict[str, Any]:
        """檢查檢查點系統"""
    ...
    def run_all_checks(self) -> SystemHealth:
        ...
        checks = [
            ...
            self.check_core_scripts(str(SCRIPT_DIR)),
...
"""
    ...
    def scan_scripts(self) -> None:
        ...
        result = self.source_validator.verify_all_core_scripts(str(SCRIPT_DIR))
    ...
    def show_status(self) -> None:
        ...
        # 磁盤空間
        try:
            usage = shutil.disk_usage(str(_PROJECT_ROOT / "var" / "kimi-agent-v2"))
            free_gb = usage.free / (1024**3)
            total_gb = usage.total / (1024**3)
    ...
    def validate_source_chain(self) -> None:
        ...
        # 驗證核心腳本
        print("\n  ─── 核心腳本DNA驗證 ───")
        script_verify = self.source_validator.verify_all_core_scripts(str(SCRIPT_DIR))
...
示例:
  python longhun_foundation_launcher_v2.0.py         # 交互式啟動台
  python longhun_foundation_launcher_v2.0.py --auto  # 自動系統檢查
  python longhun_foundation_launcher_v2.0.py --check # 單次健康檢查
        """,
    )
