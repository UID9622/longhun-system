...
# 若 8000 已被占用（如 phase3 后端），则使用 8001
if nc -z -G 1 127.0.0.1 8000 2>/dev/null; then
    MONITORING_PORT=8001
    echo -e "${YELLOW}⚠️  端口 8000 已被占用，监控服务器将使用 8001${NC}"
else
...
if [ ! -f "$MONITORING_FILE" ]; then
    ...
    start_service \
        "监控服务器 (localhost:$MONITORING_PORT)" \
...
if curl --max-time 3 -s http://localhost:$MONITORING_PORT/api/v1/monitor/health > /dev/null 2>&1; then
    ...
    if echo "$HEALTH" | grep -q "healthy"; then
        ...
        echo -e "  ${YELLOW}⚠️  localhost:$MONITORING_PORT (响应异常)${NC}"
    fi
else
    echo -e "  ${YELLOW}⚠️  localhost:$MONITORING_PORT (不可达)${NC}"
fi
