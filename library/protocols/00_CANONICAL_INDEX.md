# 龍魂协议图书馆 · 规范索引

**DNA:** `#龍芯⚡️2026-06-25-LONGHUN-CANONICAL-INDEX-v1.0`  
**GPG:** `A2D0092CEE2E5BA87035600924C3704A8CC26D5F`  
**创始人:** UID9622 / Lucky（诸葛鑫）  
**生效日期:** 2026-06-25

---

## 一、索引说明

本文件是 `library/protocols/` 的**规范索引（Canonical Index）**。  
所有入馆协议以本索引为检索入口；当协议内容与本索引发生冲突时，以协议文件正文为准，但须服从 [`L2_CONFLICT_RESOLUTION_PROTOCOL.md`](./03_governance/L2_CONFLICT_RESOLUTION_PROTOCOL.md) 的优先级裁决。

**简称**字段用于口头交流、文档互引用和命令检索。

---

## 二、L0 / L0+ 神圣文件（仓库根目录）

> **L0+** 表示“比 L0 更硬的永恒锁定”。`P0_ETERNAL_LOCK.md` 是锁的锁，用于保护宪法、标准及本协议库本身不被篡改。  
> 法律效力上，宪法为最高准绳；执行硬度上，永恒锁为最高保护。

| 文件名 | 简称 | 层级 | 目标受众 | 主题 | 状态 | DNA |
|---|---|---|---|---|---|---|
| `CONSTITUTION.md` | 宪法 | L0 | 全体成员、AI、合作方 | 龍魂系统宪法 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-CONSTITUTION-v1.2` |
| `P0_ETERNAL_LOCK.md` | 永恒锁 | L0+ | 系统核心、守护者 | P0 永恒锁协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-P0-ETERNAL-LOCK-v1.1` |
| `STANDARD.md` | 产出标准 | L0 | 全部产出方 | 龍魂产出标准 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-OUTPUT-STANDARD-v1.0` |

---

## 三、协议图书馆文件索引

| 文件名 | 简称 | 层级 | 目标受众 | 主题 | 状态 | DNA |
|---|---|---|---|---|---|---|
| `README.md` | 协议库总览 | L0 | 全部访客 | 协议图书馆总览 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-PROTOCOL-LIBRARY-v1.0` |
| `PROTOCOL_USAGE_TERMS.md` | 使用条款 | L0 | 全部使用者 | 使用条款与授权边界 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-PROTOCOL-USAGE-TERMS-v1.0` |
| `00_CANONICAL_INDEX.md` | 规范索引 | L0 | 全部使用者 | 规范索引 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-CANONICAL-INDEX-v1.0` |
| `01_foundation/SOVEREIGNTY_FORMULAS.md` | 主权公式 | L1 | 协议起草者、开发者、传播者 | 龍魂主权备注词公式集 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-SOVEREIGNTY-FORMULAS-v1.0` |
| `02_charters/L2_CREATOR_PROTECTION_CHARTER.md` | 创作者宪章 | L2 | 创作者、使用者 | 创作者保护宪章 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-CREATOR-PROTECTION-CHARTER-v1.0` |
| `02_charters/L2_CREATOR_PROTECTION_IRREVOCABLE_CLAUSES.md` | 创作者不可篡改条款 | L2 | 创作者、使用者 | 创作者保护不可篡改条款 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-CREATOR-PROTECTION-IRREVOCABLE-v1.0` |
| `02_charters/L2_OPEN_SOURCE_GENTLEMAN_CHARTER.md` | 开源君子协议 | L2 | 开源社区、合作者 | 开源君子协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-OPEN-SOURCE-GENTLEMAN-CHARTER-v1.0` |
| `02_charters/L2_ECOSYSTEM_PUBLIC_CHARTER.md` | 生态公开宪章 | L2 | 公众、平台、合作方 | 生态公开宪章 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-ECOSYSTEM-PUBLIC-CHARTER-v1.0` |
| `02_charters/L2_PEOPLE_TECHNOLOGY_MANIFESTO.md` | 人民科技宣言 | L2 | 所有人 | 龍魂人民科技宣言 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-PEOPLE-TECHNOLOGY-MANIFESTO-v1.0` |
| `03_governance/L2_CONFLICT_RESOLUTION_PROTOCOL.md` | 冲突解决协议 | L2 | 治理层、AI、仲裁者 | 协议冲突解决 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-CONFLICT-RESOLUTION-v1.0` |
| `03_governance/L2_CNSH_GOVERNANCE_SPEC.md` | CNSH治理规范 | L2 | 治理层、开发者 | CNSH 治理规范 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-CNSH-GOVERNANCE-v1.0` |
| `03_governance/L2_DATA_SOVEREIGNTY_AND_AI_ETHICS.md` | 数据主权与AI伦理 | L2 | 治理层、AI、用户 | 数据主权与 AI 伦理 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-DATA-SOVEREIGNTY-AI-ETHICS-v1.0` |
| `03_governance/L3_THREE_COLOR_AUDIT_PROTOCOL.md` | 三色审计协议 | L3 | 审计层、AI | 三色审计协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-THREE-COLOR-AUDIT-v1.0` |
| `03_governance/L3_AUDITABLE_TOOLS_PROTOCOL.md` | 可审计工具协议 | L3 | 工具开发者、AI | 可审计工具协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-AUDITABLE-TOOLS-v1.0` |
| `04_agents/L3_AI_COLLABORATION_PROTOCOL.md` | AI协作协议 | L3 | AI、用户、守护者 | 人机协作协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-AI-COLLABORATION-v1.0` |
| `04_agents/L3_AGENT_COMMUNICATION_PROTOCOL.md` | Agent通信协议 | L3 | AI 智能体 | 智能体通信协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-AGENT-COMMUNICATION-v1.0` |
| `05_cnsh/L4_CNSH_P0_ETERNAL_EMBED_PROTOCOL.md` | CNSH-P0嵌入 | L4 | CNSH 运行时、人格 | CNSH-P0 永恒嵌入 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-CNSH-P0-ETERNAL-EMBED-v1.0` |
| `05_cnsh/L4_CNSH_HUMAN_FIRST_COLLABORATION.md` | 人性优先协作 | L4 | CNSH 用户、开发者 | 人性优先协作框架 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-CNSH-HUMAN-FIRST-v1.0` |
| `06_individual/L5_TERMS_OF_SERVICE.md` | 用户服务条款 | L5 | 个人用户 | 服务条款 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-TERMS-OF-SERVICE-v1.0` |
| `06_individual/L5_PRIVACY_POLICY.md` | 隐私政策 | L5 | 个人用户 | 隐私政策 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-PRIVACY-POLICY-v1.0` |
| `06_individual/L5_MINOR_PROTECTION_POLICY.md` | 未成年人保护 | L5 | 未成年人及其监护人 | 未成年人保护 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-MINOR-PROTECTION-v1.0` |
| `07_enterprise/L6_COMMERCIAL_LICENSE_AGREEMENT.md` | 商业许可 | L6 | 企业、商业用户 | 商业许可协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-COMMERCIAL-LICENSE-v1.0` |
| `07_enterprise/L6_DATA_PROCESSING_AGREEMENT.md` | 数据处理协议 | L6 | 企业、数据处理方 | 数据处理协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-DATA-PROCESSING-v1.0` |
| `07_enterprise/L6_SERVICE_LEVEL_AGREEMENT.md` | SLA | L6 | 企业、服务提供方 | 服务等级协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-SERVICE-LEVEL-v1.0` |
| `07_enterprise/L6_INCIDENT_RESPONSE_PROTOCOL.md` | 事件响应 | L6 | 企业、运维、安全 | 事件响应协议 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-INCIDENT-RESPONSE-v1.0` |
| `08_international/L7_CROSS_BORDER_DATA_HANDLING.md` | 跨境数据 | L7 | 跨境使用者 | 跨境数据处理 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-CROSS-BORDER-DATA-v1.0` |
| `08_international/L7_INTERNATIONAL_COMPLIANCE_STATEMENT.md` | 国际合规声明 | L7 | 国际用户、合作方 | 国际合规声明 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-INTERNATIONAL-COMPLIANCE-v1.0` |
| `08_international/L7_DISPUTE_RESOLUTION_POLICY.md` | 争端解决 | L7 | 国际用户、合作方 | 争议解决政策 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-DISPUTE-RESOLUTION-v1.0` |
| `09_copyright/L8_COPYRIGHT_AND_ANTI_HARVESTING_PROTOCOL.md` | 版权反收割 | L8 | 创作者、平台、用户 | 版权与反收割 | 生效中 | `#龍芯⚡️2026-06-25-LONGHUN-COPYRIGHT-ANTI-HARVESTING-v1.0` |
| `templates/protocol_document_template.md` | 协议模板 | — | 协议起草者 | 协议文档模板 | 生效中 | `#龍芯⚡️YYYY-MM-DD-LONGHUN-PROTOCOL-NAME-v1.0` |
| `templates/protocol_proposal_template.md` | 提案模板 | — | 提案提交者 | 协议提案模板 | 生效中 | `#龍芯⚡️YYYY-MM-DD-LONGHUN-PROPOSAL-NAME-v1.0` |

---

## 四、阅读路径建议

不同身份的人，不需要读完所有协议，从下列入口开始即可。

### 4.1 创始人 UID9622 / 普通用户

| 顺序 | 阅读文件 | 简称 |
|---|---|---|
| 1 | `README.md` | 协议库总览 |
| 2 | `L2_PEOPLE_TECHNOLOGY_MANIFESTO.md` | 人民科技宣言 |
| 3 | `L5_PRIVACY_POLICY.md` | 隐私政策 |
| 4 | `L5_TERMS_OF_SERVICE.md` | 用户服务条款 |

**创始人不需要背诵，遇到问题直接问：** `lh 问协议 "企业想用我数据怎么办"`

### 4.2 创作者 / 开源贡献者

| 顺序 | 阅读文件 | 简称 |
|---|---|---|
| 1 | `L2_PEOPLE_TECHNOLOGY_MANIFESTO.md` | 人民科技宣言 |
| 2 | `L2_CREATOR_PROTECTION_CHARTER.md` | 创作者宪章 |
| 3 | `L2_OPEN_SOURCE_GENTLEMAN_CHARTER.md` | 开源君子协议 |
| 4 | `L8_COPYRIGHT_AND_ANTI_HARVESTING_PROTOCOL.md` | 版权反收割 |

### 4.3 企业 / 商业合作方

| 顺序 | 阅读文件 | 简称 |
|---|---|---|
| 1 | `PROTOCOL_USAGE_TERMS.md` | 使用条款 |
| 2 | `L6_COMMERCIAL_LICENSE_AGREEMENT.md` | 商业许可 |
| 3 | `L6_DATA_PROCESSING_AGREEMENT.md` | 数据处理协议 |
| 4 | `L6_SERVICE_LEVEL_AGREEMENT.md` | SLA |
| 5 | `L7_CROSS_BORDER_DATA_HANDLING.md` | 跨境数据 |

### 4.4 AI 智能体 / 系统执行者

| 顺序 | 阅读文件 | 简称 |
|---|---|---|
| 1 | `CONSTITUTION.md` | 宪法 |
| 2 | `P0_ETERNAL_LOCK.md` | 永恒锁 |
| 3 | `L3_AI_COLLABORATION_PROTOCOL.md` | AI协作协议 |
| 4 | `L2_CONFLICT_RESOLUTION_PROTOCOL.md` | 冲突解决协议 |
| 5 | `L3_THREE_COLOR_AUDIT_PROTOCOL.md` | 三色审计协议 |

### 4.5 国际用户 / 跨境合作方

| 顺序 | 阅读文件 | 简称 |
|---|---|---|
| 1 | `L7_INTERNATIONAL_COMPLIANCE_STATEMENT.md` | 国际合规声明 |
| 2 | `L7_DISPUTE_RESOLUTION_POLICY.md` | 争端解决 |
| 3 | `L7_CROSS_BORDER_DATA_HANDLING.md` | 跨境数据 |

---

## 五、层级速查

| 层级 | 定位 | 类比 |
|---|---|---|
| L0+ | 永恒锁 | 系统的“保险箱密码” |
| L0 | 宪法、标准、总览 | 系统的“天条” |
| L1 | 主权公式集 | 系统的“数学公理” |
| L2 | 宪章、宣言、治理规范 | 系统的“价值主张” |
| L3 | 审计、AI 协作、Agent 通信 | 系统的“执行机制” |
| L4 | CNSH 技术协议 | 系统的“技术落地” |
| L5-L6 | 个人/企业协议 | 系统的“对外接口” |
| L7-L8 | 国际/版权协议 | 系统的“主权边界” |

---

## 六、使用提示

> 本文件归龍魂系统所有，未经许可不得修改。  
> 本索引从属于 `CONSTITUTION.md`，如有冲突以宪法为准。

---

**DNA:** `#龍芯⚡️2026-06-25-LONGHUN-CANONICAL-INDEX-v1.0`

---

## 龍魂主权与法律效力声明

本协议的一切权利、义务与责任，均由《龍魂系统宪法》及中华人民共和国法律最终解释与保障。  
任何境外法律、外国法院裁判、国际规则或境外组织章程，均不得凌驾于本协议之上。  
未经授权，任何人不得修改、演绎、转售或去除本协议中的 DNA 与主权声明。
