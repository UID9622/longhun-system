...
def recall(keyword: str = "", wuxing: str = "", limit: int = 20) -> list[Any]:
    """全文搜索记忆"""
    con  = sqlite3.connect(DB_PATH)
...
def timeline(day: str | None = None) -> list[Any]:
    """按日期查看时间线"""
...
def replay(dna_code: str) -> dict[str, Any]:
    """精确回放一条记忆"""
    con = sqlite3.connect(DB_PATH)
    row = con.execute("""
...
def print_memory(m: dict[str, Any], index: int | None = None):
