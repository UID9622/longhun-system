...
def CNSH_命名合规(路径: str) -> dict[str, Any]:
    """
    对路径逐段判定人读层中文覆盖率 γ。
    硬要求 γ = 1.0。
    发现裸英文主名 → 🔴 拒收 + 给出v3迁移建议。
...
def CNSH_双视角一致(m_status: str, c_policy: str) -> dict[str, Any]:
    """
...
def CNSH_双版同步(中文版本: str, 镜像版本: str, 中文段数: int, 镜像段数: int) -> dict[str, Any]:
    """
    版本锁: V(中文) = V(镜像)，不等即 🔴。
    结构偏差率 δ ≤ 0.10。
    """
...
def CNSH_双段封装(m段: Optional[str], c段: Optional[str]) -> dict[str, Any]:
    """M::×CNSH:: 双段缺一即拒"""
    if not m段 or not c段:
...
def 打印报告(标题: str, 数据: dict[str, Any], 缩进: int = 0):
    前缀 = "  " * 缩进
    print(f"{前缀}━━ {标题} ━━")
    for k, v in 数据.items():
