...
# 龍魂体系 | 压缩卡模块 v1.0
# ═══════════════════════════════════════════
# DNA: #龍芯⚡️2026-07-20-COMPRESSION-CARD-v1.0
...
class 压缩卡:
    ...
    @classmethod
    def from_dict(cls, d: dict[str, Any]) -> "压缩卡":
        return cls(**{k: v for k, v in d.items() if k in cls.__dataclass_fields__})

    # ═══════════════════════════════════════
    # M:: 机器块（结构化·可程序解析）
    # ═══════════════════════════════════════
    def M块(self) -> dict[str, Any]:
        return {
            "id": self.id,
            "一句话": self.一句话,
            "核心结论": self.核心结论,
...
class 压缩卡管理器:
    ...
    def 创建压缩卡(self, 来源文件: str = "", 原文: str = "", 档: str = "常规",
                   压缩引擎=None, 用户标记: int = 5, 情感: float = 0.5) -> dict[str, Any]:
        """从原文生成一张完整压缩卡"""
        if 压缩引擎 is None:
    ...
    def 导出索引CSV(self, path: str | None = None) -> Path:
        if path is None:
