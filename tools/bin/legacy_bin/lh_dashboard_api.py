...
def check_port(host: str = "localhost", port: int = 0, timeout: float = 2.0) -> dict[str, Any]:
    """检查端口是否可达"""
    try:
...
def get_services_status() -> list[Any]:
    """获取所有服务状态"""
    results = []
    for svc in SERVICES:
...
def get_recent_logs(lines: int = 20) -> list[Any]:
    """获取最近日志"""
    entries = []
    log_files = [
...
def action_restart_services(target: str = "all") -> dict[str, Any]:
    """重启服务"""
    # 安全确认
...
def _restart_service(svc_id: str) -> dict[str, Any]:
    """重启单个服务"""
...
def register_routes(app):
    ...
    async def dashboard_action(action: str, target: str | None = None, confirm_code: str | None = None):
        # 验证确认码
