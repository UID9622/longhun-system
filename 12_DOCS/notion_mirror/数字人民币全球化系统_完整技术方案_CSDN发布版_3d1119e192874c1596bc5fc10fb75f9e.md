...
> Notion URL: https://app.notion.com/p/CSDN-3d1119e192874c1596bc5fc10fb75f9e
> Created: 2025-12-16T15:56:00.000Z
...
---
## 📖 系统概述
CNSH数字人民币全球化系统是一个基于易经推演、数学建模和区块链技术的跨境支付解决方案。
### 核心特点
- 🇨🇳 主权货币优先 - 数字人民币(e-CNY)为「干净」标杆；各国法币（含美金）只要达到同一标准——每笔直达·点对点·不走第三方·可追溯——皆可在生态流动（升级自旧「唯一结算」表述；详见货币·文化主权原则页 / "only" means clean · traceable · capital-deterring, not exclusion of other legal tenders）
...
## 💻 核心代码实现
### 1. 易经推演引擎
文件：backend/app/services/yijing_engine.py
```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
---
### 2. 数学增长模型
文件：backend/app/services/growth_model.py
```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
# 3. 获取八卦战略
curl http://localhost:8000/api/yijing/strategy

# 4. 易经占卜
curl -X POST http://localhost:8000/api/yijing/divine \
  -H "Content-Type: application/json" \
...
# 6. 蒙特卡洛模拟
curl http://localhost:8000/api/growth/montecarlo?iterations=10000
```
### 使用Python测试
```python
import requests
...
```bash
# 数据库配置
DATABASE_URL=postgresql://ecny_user:your_password@localhost:5432/ecny_db
...
开发流程：
1. Fork本项目
1. 创建feature分支：git checkout -b feature/your-feature
1. 配置Git签名（必需）：
步骤A：安装GPG工具
```bash
# Ubuntu/Debian
...
# - 电子邮件：输入 "uid9622@petalmail.com"
# - 注释：可以留空或输入 "CNSH龍魂数字身份"
# - 密码：设置一个强密码（至少12位）
```
步骤C：查看并记录密钥信息
```bash
...
#       1234567890ABCDEF1234567890ABCDEF12345678  ← 这是完整指纹
# uid                 [ultimate] 💎 Lucky｜UID9622 <uid9622@petalmail.com>
# ssb   rsa4096/1234567890ABCDEF 2025-12-17 [E]

# 记住两个重要信息：
# 1. 密钥ID：ABCD1234EFGH5678（sec后面的部分）
# 2. 完整指纹：1234567890ABCDEF1234567890ABCDEF12345678（40位）
```
步骤D：配置Git使用GPG
```bash
# 3. 配置用户信息
git config user.name "💎 Lucky｜UID9622"
...
```bash
# 6. 导出公钥
gpg --armor --export uid9622@petalmail.com
...
# 应该看到：
# gpg: Signature made ...
# gpg: Good signature from "💎 Lucky｜UID9622 <uid9622@petalmail.com>"
```
---
⚠️ 常见问题解决
...
```bash
# 查看完整指纹：
gpg --fingerprint uid9622@petalmail.com
...
```
### 合法签名者
- 💎 Lucky｜UID9622 <uid9622@petalmail.com>
