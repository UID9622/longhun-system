...
| **跳跃型重复** | 同一主题但跨维度展开 | 标记为「思维跳跃碎片」，存入本地拼图池，等待合龙 |
    ...
    def detect_closure(self, fragment_pool):
        ...
        for cycle in cycles:
            report = {
                ...
                "coverage": self.check_coverage(cycle),  # 已落地/还缺/可合龙
...
**核心原则**：老大的思维碎片，纯本地存储，永不上云。单个碎片对外毫无价值，合龙后才对内可见。
