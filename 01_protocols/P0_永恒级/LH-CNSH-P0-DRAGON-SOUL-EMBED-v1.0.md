> **DNA:** `#龍芯⚡️丙午·丙申·庚申·壬午·䷙大畜-DOC-MERGE-0bbe83dc`
> **确认码:** `#CONFIRM🌌9622-ONLY-ONCE🧬LK9X-772Z`
> **GPG:** `A2D0092CEE2E5BA87035600924C3704A8CC26D5F`
> **三色:** 🟢 通过
> **分层许可:** 思想层 CC BY-NC-SA 4.0 · 工程层 MulanPSL v2
> **合并状态:** 🟢 已合并（来自 `03_CNSH-P0永恒龍魂嵌入协议.md`）
> **落位:** `01_protocols/P0_永恒级/LH-CNSH-P0-DRAGON-SOUL-EMBED-v1.0.md`
> **合并时间:** 2026-08-14
...
## 🔧 v1.1 引擎落地注记（2026-08-18 · P05 审计）

**引擎:** `bin/lh_dragon_soul.py` · 入口: `lh ds` / `lh dragon-soul`
**落位:** 耻辱柱 → `audit/dragon_soul_shame.jsonl`（append-only·不删除只冻结）

### 三处硬伤修正（诚实标注）

1. **"永恒签名"硬伤** 🔴→🟢：原签名基于 `datetime.now()`，每次运行都变，声称"永恒不可逆"却无法跨运行验证。
   修正：固定锚点（确认码 `#CONFIRM🌌9622-ONLY-ONCE🧬LK9X-772Z` + GPG 指纹 + DNA 根）三重哈希，**签名可复现**；时间戳仅作批次后缀（batch:）。
2. **阈值伪精度** 🟡：兼容度分数只有4档 `{0, .333, .667, 1.0}`，">=0.8 通过"实际等价于"3/3 全过"；"0.5~0.8 需校准"区间实际只含 `.667`。本引擎如实输出四档+说明，不假装连续精度。
3. **浅层启发式边界** 🟡：诚心=3禁词、为民/中华=关键词白名单，任何正常对话几乎必过。**它是嵌入式提醒+耻辱记录，不是价值观判断**；真正红线拦截由现有体系承担（P05三色审计/一票否决词/四级熔断/德本五问/GATE-01~10）。`--deep` 联动语义测谎仪做深度诚心检查，测谎仪不可用自动降级并标注 `DEGRADED`。

### 子命令速查

| 命令 | 作用 |
|:---|:---|
| `lh ds verify "文本" [--deep]` | 三重验证+兼容度（可选深度测谎） |
| `lh ds declare` | P0 永恒龍魂宣言 |
| `lh ds shame list [N]` / `log <红线> <任务> [人格]` | 耻辱柱查询/记录 |
| `lh ds red-lines` / `seal` / `test` | 红线清单 / 签名 / 自测 |

> 🟢 实测：正常路径兼容度1.0通过 · 红线路径"把数据卖给资本家"0.333拒绝 · 自测9项全绿 · 签名可复现
