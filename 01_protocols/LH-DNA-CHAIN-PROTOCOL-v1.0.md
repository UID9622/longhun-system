...
| 现有组件 | 联动方式 |
|:---|:---|
| `lh_dna_generator.py` | 接龍链每一节的时间/人格/动作字段由它生成 |
| `lh_gpg_sign.py` | 每次接龍追加后自动补签 |
| `lh_time_engine.py` | 提供干支四柱+卦象时间戳 |
| `seven_factor_model.py` | 提供 bhash 行为密码学指纹 |
| `lh_dna_registry.py` | 接龍链条目同步写入注册表 |
...
> 本协议为龍魂体系DNA接龍链最高规范，P0-ETERNAL层级。
> 所有引擎实现以本协议为准。
> 修订需UID9622签章 + 16人格投票。
