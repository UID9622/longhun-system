...
龍魂系统不是单一AI模型，不是单一服务。它是一个由42+龙魂技能、20人格Agent、数百引擎、千级文件构成的生命体。生命体的核心挑战不是"能做多少"，而是——**每个部件能否说同一种语言、按同一种节奏呼吸？**
...
def check_dimension_data(files: list) -> dict:
    ...
    for f in files:
        ...
        if "龙魂" in content and f.endswith((".md", ".py", ".sh")):
            issues.append(f"❌ {f}: 发现简体「龙魂」，应改为繁体「龍魂」")
