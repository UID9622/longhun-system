...
**真实运行结果示例（2026-09-06 10:47 实跑输出 · 直接复制你的终端输出）：**
...
> ⚠️ **注意1**：DNA 哈希（上例 `6A07B196`）与 timestamp 随运行时间变化——**你实际运行得到的值才是你的证据**，禁止照抄本示例。
> ⚠️ **注意2**：`extract_token_f1=0.690948` 是真实值（示例抽取数据非完全匹配），**不是 1.0**——任何「全 1.0」的干净输出都值得怀疑，防空壳标准拒绝手写理想输出。
