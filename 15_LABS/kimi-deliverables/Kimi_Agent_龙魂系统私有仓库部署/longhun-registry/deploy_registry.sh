...
# 署名：龍芯北辰 UID9622
#
# deploy_registry.sh — 龍魂私有 Docker 镜像仓库 · 鲲鹏(aarch64)一键部署
# 用法：在鲲鹏服务器上执行  bash deploy_registry.sh
# 参考方案：[快速链接: CSDN 参考文章] https://blog.csdn.net/Margrop/article/details/163312205
#           （registry:2 + htpasswd + restart=always，本文为其 ARM64/鲲鹏适配增强版）
...
# ---------- 第 1 步：检测架构，非 aarch64 直接退出 ----------
ARCH="$(uname -m)"
if [ "$ARCH" != "aarch64" ]; then
  echo "[错误] 本脚本仅支持华为鲲鹏 ARM64(aarch64)，当前架构为：$ARCH" >&2
  echo "       如在 x86 服务器部署，请使用 x86 版部署脚本。" >&2
  exit 1
fi
echo "[1/8] 架构检测通过：aarch64（鲲鹏）"

# ---------- 第 2 步：检测/安装 Docker（openEuler / EulerOS 用 dnf） ----------
if ! command -v docker >/dev/null 2>&1; then
  echo "[2/8] 未检测到 Docker，正在安装（dnf）..."
...
# ---------- 第 6 步：拉取 registry:2 并校验 arm64 manifest ----------
echo "[6/8] 拉取 registry:2（官方镜像，manifest 含 linux/arm64）..."
docker pull --platform linux/arm64 registry:2
# 校验本地镜像架构，非 arm64 立即终止
IMG_ARCH="$(docker image inspect registry:2 --format '{{.Architecture}}')"
if [ "$IMG_ARCH" != "arm64" ]; then
  echo "[错误] registry:2 镜像架构为 $IMG_ARCH，非 arm64，终止。" >&2; exit 1
fi
echo "[6/8] registry:2 arm64 镜像校验通过"
