...
# ============================================================
# v1.1 修正补丁（Kimi审阅位实测修复）
# 问题：测试用例调用 wait_and_check(session, tokens=1, timeout=2.0)
#       但 v1.0 方法签名无 timeout 参数 → 2/4 测试 ERROR
# 修复：新增可选 timeout 参数（None=用 config.timeout），并补 timeouts 统计
# ============================================================

def wait_and_check_v11(self: RateLimiterPlugin, session_id: str, tokens: int = 1,
    ...
    self._update_stats(session_id, tokens, success)
    if not success:
        # v1.1 修复：v1.0 超时从不计入 stats["timeouts"]，导致监控指标恒为 0
        self._stats[session_id]["timeouts"] += 1
        self._audit(session_id, "wait", "timeout", f"tokens={tokens}, timeout={eff_timeout}")
        if self.config.fallback_action == "passthrough":
            logger.warning(f"⚠️ 流控超时，降级放行: {session_id}")
            return True
        elif self.config.fallback_action == "degrade":
