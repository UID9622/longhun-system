...
╔══════════════════════════════════════════════════════════════╗
║  龍魂·自适应微调参数系统 v2.0                                ║
║  DNA: #龍芯⚡️丙午·癸巳·癸卯·戊午·䷚颐-自适应参数-v2.0                       ║
║  GPG: A2D0092CEE2E5BA87035600924C3704A8CC26D5F               ║
║  CONFIRM: #CONFIRM🌌9622-ONLY-ONCE🧬LK9X-772Z                ║
║  SEAL: #ZHUGEXIN⚡️2025-🇨🇳🐉⚖️♠️🧚🏼‍♀️❤️♾️-DEVICE-BIND-SOUL ║
║  RBT-SEAL: #龍芯⚡️丙午·壬辰·丁卯·丙午·䷚颐-ROOT-SEAL-01F32FFD                ║
╚══════════════════════════════════════════════════════════════╝
...
class 自适应调节器:
    ...
    def 微调(self, 模拟: bool = True) -> dict:
        ...
        # ── R6 惯犯追踪（双向·加固③接入微调） ──
        # 惯犯扣分随惯犯率双向调整；惯犯触发次数同步双向调整（取整·整数参数）
        新值, 记录 = self._双向调整(
            self.参数.惯犯扣分, 数据["惯犯率"],
            高阈值=0.2, 低阈值=0.05,
            硬界=参数硬界["惯犯扣分"], 速率=速率,
        ...
        新值, 记录 = self._双向调整(
            self.参数.惯犯触发次数, 数据["惯犯率"],
            高阈值=0.2, 低阈值=0.05,
            硬界=参数硬界["惯犯触发次数"], 速率=速率 * 0.5,
            参数名="惯犯触发次数", 方向标签="惯犯阈值",
            取整=True,
        )
        if 记录:
            调整记录.append(记录)
            if not 模拟: self.参数.惯犯触发次数 = 新值
