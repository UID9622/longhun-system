...
class SelfHealEngine:
    ...
    def plan(self, errors: list[DetectedError]) -> list[dict]:
        ...
        dep_missing("ModuleNotFoundError: X") → pip install -- X（白名单校验包名 ^[a-zA-Z0-9][a-zA-Z0-9_.-]*$）
...
```
CLI：`python3 -m longhun_trust.selfheal --once|--status [--execute] [--confirm-code X] [--project-root P] [--log-dir D] [--ports 8080,9090]`
退出码 = HealStatus 值（越界一律 clamp 到 2；未处理异常 → 错误摘要 + exit 2）。
...
- 锚点3：`compute_credibility(0, FOUNDER, CONFIRMED) == 1.0`；
  `compute_credibility(45, FOUNDER, CONFIRMED) == 0.8`（F=0.5→0.4·0.5+0.3+0.3=0.8，按公式实算）
