...
LOG_FILE="${LOG_DIR}/skill-launcher-$(date +%Y%m%d-%H%M%S).log"
PID_FILE="${LOG_DIR}/.skill-launcher.pid"
...
check_environment() {
    ...
    if [[ -d "${ASSETS_DIR}" ]]; then
        log OK "资源目录: ${ASSETS_DIR} 存在 ${AUDIT_PASS}"
    else
        log WARN "资源目录: ${ASSETS_DIR} 不存在 ${AUDIT_WARN}"
        log INFO "将尝试从当前目录查找..."
        ASSETS_DIR="${LAUNCHER_DIR}"
    fi
