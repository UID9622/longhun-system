...
class CNSH_DNA绑定防御器:
    ...
    # 5.3 血缘哈希链
    # ═══════════════════════════════════════
    def 铸造DNA(self, 内容: str, 父DNA: str = None, 文件路径: str = "") -> str:
        """产出即捆绑：每个文件诞生自动铸造DNA"""
        self._seq += 1
    ...
    def 验链(self, dna_codes: Optional[list] = None, 全量审计: bool = False) -> dict:
    ...
    def 触发集判定(self, 命中数: int, m: int = M_TRIG, p0: float = P0_TRIG) -> dict:
    ...
    # 7.1 产出即捆绑
    # ═══════════════════════════════════════
    def 绑定文件(self, 文件路径: str, 级别: str = "B1", 父DNA: str = None) -> dict:
        p = Path(文件路径)
        if not p.exists():
    ...
    def 绑定目录(self, 目录: str, 级别: str = "B1", 扩展名: list = None) -> dict:
        """批量绑定目录下所有文件"""
        dir_path = Path(目录)
    ...
    # 7.4 资产总账
    # ═══════════════════════════════════════
    def 总账(self, 级别过滤: str = None) -> list:
        if 级别过滤:
    ...
    # 6.4 四步取证流程（记录）
    # ═══════════════════════════════════════
    def 取证记录(self, case_id: str, evidence: dict, result: str):
        self._query(
