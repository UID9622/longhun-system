...
#!/usr/bin/env python3
#龍芯⚡️2026-07-19-MODEL-LORA-TRAINER-v4.0.8
# CREATOR: 诸葛鑫 (UID9622)
...
用法:
  ...
  python3 bin/lh_lora_trainer_v408.py prepare  # 准备训练数据
  python3 bin/lh_lora_trainer_v408.py train    # 开始 LoRA 微调
  python3 bin/lh_lora_trainer_v408.py fuse     # 合并 adapter
  python3 bin/lh_lora_trainer_v408.py export   # 导出 GGUF → Ollama
  python3 bin/lh_lora_trainer_v408.py test     # 快速测试
"""
...
class Config:
    ...
    model_name = "longhun-v4.0.8-lora"
    ...
    output_dir = project_root / "models" / "longhun-v1.0" / "lora_output_v408"

    # 数据：合并 v4.0.7 + 八卦阵 v1.1 + 道德经定锚 v1.1 + 水军显化 v1.2
    memory_data_dir = PROJECT / "models" / "longhun-v1.0" / "memory_ingested_data_v1.0"
    data_dir = output_dir / "data_v408"

    adapter_dir = output_dir / "adapter_v408"
    merged_dir = output_dir / "merged_v408"
    gguf_dir = output_dir / "gguf_v408"
...
def setup_model():
    ...
    print("🛠️  设置 v4.0.8 底模: Yi-1.5-9B-Chat → MLX")
    cfg = Config()
...
def prepare_data():
    """数据准备：使用已合并的 v4.0.8 数据集。"""
    print("📝 准备 v4.0.8 训练数据（v4.0.7 + 八卦阵 v1.1 + 道德经定锚 v1.1 + 水军显化 v1.2 + 核心强化）...")
    cfg = Config()
    cfg.output_dir.mkdir(parents=True, exist_ok=True)

    train_src = cfg.memory_data_dir / "train_v408_merged.jsonl"
    val_src = cfg.memory_data_dir / "valid_v408_merged.jsonl"

    if not train_src.exists() or not val_src.exists():
        print(f"   ❌ 合并数据不存在，请先运行:")
        print(f"      python3 bin/lh_merge_v408_dataset.py")
        sys.exit(1)
    ...
    print(f"   ✅ v4.0.8 数据就绪: {work_data_dir}")
    print(f"   总训练样本: {len(train)} | 总验证样本: {len(valid)}")
    print(f"   域分布: {dict(domains.most_common(10))}")


def export_gguf():
    """导出 GGUF → Ollama，模型名 longhun-v4.0.8"""
    print("📦 导出 GGUF v4.0.8...")
    cfg = Config()
    ...
    gguf_path = cfg.gguf_dir / "longhun-v4.0.8.F16.gguf"
...
SYSTEM \"\"\"
你是龍魂 longhun-v4.0.8，基于 01-ai/Yi-1.5-9B-Chat 用龍魂系统自有语料 LoRA 微调（底座已非 Qwen）。
...
""")
    ...
    print(f"\n🐉 部署到 Ollama:")
    print(f"   ollama create longhun-v4.0.8 -f {modelfile}")
    subprocess.run(["ollama", "create", "longhun-v4.0.8", "-f", str(modelfile)], check=True)
    print(f"   ✅ Ollama 模型 longhun-v4.0.8 已创建")


def test_model():
    print("🧪 测试 longhun-v4.0.8...")
    import requests
    prompts = [
        ("你是谁？", "身份认知"),
        ("什么是家法第一条？", "家法主权"),
    ...
    for prompt, label in prompts:
        try:
            r = requests.post(
                ...
                json={"model": "longhun-v4.0.8", "prompt": prompt, "stream": False},
                timeout=60
            )
