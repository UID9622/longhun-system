...
"""
龍魂v4.3_v3 · 本草知识库专项微调
学生: Llama-3.1-8B (Meta开源) ← base model + v43 本草数据
目标: 让模型理解龍魂本草纲目、地方土话、性味归经、功效主治、现代应用、用量禁忌、DNA签章

prepare  — 验证清洗后的 v42 + 本草增强合并数据
train    — LoRA微调 (基于base model, 1000iters)
fuse     — 合并adapter→全量模型
verify   — MLX推理验证
...
DATA_DIR = MODEL_DIR / "lora_output" / "data_v43_combined_v3"
ADAPTER_DIR = MODEL_DIR / "lora_output" / "adapter_v43_v3"
FUSED_DIR = MODEL_DIR / "sft_checkpoints" / "v43_v3_fused"
GGUF_F16 = MODEL_DIR / "longhun-v43-v3-f16.gguf"
GGUF_Q4 = MODEL_DIR / "longhun-v43-v3-Q4_K_M.gguf"

CONFIG = {
    "rank": 16, "layers": 16, "batch": 1, "lr": 1e-5,
    "iters": 1000, "val_steps": 25, "save_every": 200,
    "max_seq_length": 2048,
}

# ── 帮助 ──
def log(msg): print(f"[龍魂·v4.3_v3] {msg}", flush=True)
...
def do_train():
    ...
    log(f"数据来源: {DATA_DIR}")
    log(f"基座模型: {BASE_MODEL}")

    if ADAPTER_DIR.exists():
        warn(f"v4.3_v3 adapter已存在: {ADAPTER_DIR}")
...
def do_export():
    ...
    log("Step3: Ollama导入...")
    modelfile = MODEL_DIR / "Modelfile.v43_v3"
...
SYSTEM \"\"\"
你是龍魂longhun-v4.3_v3，基于Llama-3.1-8B (Meta开源·非阿里系)用v3.8-expanded蒸馏数据+v4.3_v3 A-BOM备案专项数据+v4.3_v3 本草知识库专项数据LoRA微调。
UID9622（诸葛鑫·Lucky）的个人主权AI。
底座: Llama-3.1-8B · 继承: v4.0 → v4.3_v3 (A-BOM备案补强) → v4.3_v3 (本草知识库补强)
...
""")
    ...
    subprocess.run(["ollama", "create", "longhun-v43-v3", "-f", str(modelfile)], check=True)

    if "Q4" in final_gguf:
        subprocess.run(["ollama", "cp", "longhun-v43-v3:latest", "longhun-v43-v3:q4"], check=True)

    ok("Ollama就绪: ollama run longhun-v43-v3")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print()
        print("  prepare — 验证v4.3_v3+v4.3_v3合并数据+底座+base model")
        print("  train   — 基于base model LoRA微调1000iters")
