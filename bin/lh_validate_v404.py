...
#!/usr/bin/env python3
#龍芯⚡️丙午·乙未·甲寅·未时·乾-V404-VALIDATION-REPORT-v1.0
# CREATOR: 诸葛鑫 (UID9622)
# PROTOCOL: CC BY-NC-SA 4.0
# -*- coding: utf-8 -*-
"""
v4.0.4 部署验证脚本
DNA: #龍芯⚡️丙午·乙未·甲寅·未时·乾-V404-VALIDATION-REPORT-v1.0
"""
...
DATA_DIR = PROJECT / "models" / "longhun-v1.0" / "lora_output_v404" / "data_v404"
REPORT_DIR = PROJECT / "models" / "longhun-v1.0" / "lora_output_v404" / "validation_reports"
REPORT_DIR.mkdir(parents=True, exist_ok=True)

OLLAMA_GENERATE_URL = "http://localhost:11434/api/generate"
OLLAMA_CHAT_URL = "http://localhost:11434/api/chat"
MODEL = "longhun-v4.0.4"
V37_BASELINE = 0.194
V392_BEST = 0.496
V402_BEST = 0.496
V403_BEST = 0.824
...
def load_v404_val_loss():
    loss_file = PROJECT / "models" / "longhun-v1.0" / "lora_output_v404" / "adapter_v4.0_best" / "val_loss.json"
    if loss_file.exists():
        return json.loads(loss_file.read_text()).get("best_val_loss", V403_BEST)
    return V403_BEST


def val_loss_audit():
    print("📊 Val Loss 对比审计")
    v404_best = load_v404_val_loss()
    delta = V37_BASELINE - v404_best
    if v404_best < V37_BASELINE * 0.9:
        color = "🟢 进步"
    elif v404_best <= V37_BASELINE:
        color = "🟡 持平"
    else:
        color = "🔴 退步"
    print(f"   v3.7 基线: {V37_BASELINE}")
    print(f"   v3.9.2 (1.5B): {V392_BEST}")
    print(f"   v4.0.2/3 (Llama 8B): {V403_BEST}")
    print(f"   v4.0.4 (Yi 9B): {v404_best}")
    print(f"   差值(v3.7→v4.0.4): {delta:+.4f} → {color}")
    return {"v37": V37_BASELINE, "v392": V392_BEST, "v403": V403_BEST, "v404": v404_best, "delta": delta, "color": color}


def generate_report(recall, failed, drift_score, history, loss_audit, total_samples=0):
    now = time.strftime("%Y-%m-%d %H:%M:%S")
    report_path = REPORT_DIR / "v4.0.4_validation_report.md"
    recall_color = "🟢" if recall >= 0.95 else ("🟡" if recall >= 0.85 else "🔴")
    drift_color = "🟢" if drift_score >= 0.95 else ("🟡" if drift_score >= 0.85 else "🔴")
    md = f"""# 龍魂 v4.0.4 部署验证报告
...
| 变化(v3.7→v4.0.4) | {loss_audit['delta']:+.4f} | **{loss_audit['color']}** |
