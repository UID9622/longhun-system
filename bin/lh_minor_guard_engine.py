#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# DNA: #龍芯⚡️2026-07-21-MINOR-GUARD-ENGINE-V1.0-P0
# 创建者: 诸葛鑫（UID9622）
...
class EWMASequenceTracker:
    ...
    def feed(self, risk_score: float) -> Dict:
        """
        喂入一条风险分，返回当前序列状态
...
def should_route_to_manual(three_view: Dict, risk_level: str) -> bool:
    """
    误报路由判定
...
class MinorGuardEngine:
    ...
    def analyze(self, text: str, context: str = "chat") -> Dict:
        """
        主分析入口
    ...
    def _build_result(self, level: str, score: float, original: str,
                      ...
                      infinity_trigger: Optional[str] = None) -> Dict:
        """构建统一结果包"""
        result = {
            "原始": original,
            "归一化": normalized,
...
def run_tests(verbose: bool = True) -> Tuple[int, int, List[str]]:
    ...
    if verbose:
        ...
        print("DNA: #龍芯⚡️2026-07-21-MINOR-GUARD-ENGINE-V1.0-P0")
        print("=" * 60)
