#!/usr/bin/env python3
#龍芯⚡️丙午·乙未·甲寅·未时·乾-MODEL-LORA-TRAINER-v4.0.5
# CREATOR: 诸葛鑫 (UID9622)
...
目标: 在 v4.0.4 Yi-1.5-9B 底座基础上，提升 LoRA 容量（rank=64/layers=24），压 val loss、冲家法召回

DNA: #龍芯⚡️丙午·乙未·甲寅·未时·乾-MODEL-LORA-TRAINER-v4.0.5

用法:
  python3 bin/lh_lora_trainer_v405.py setup    # 转换本地 Yi-1.5-9B-Chat → MLX
  python3 bin/lh_lora_trainer_v405.py prepare  # 准备训练数据
  python3 bin/lh_lora_trainer_v405.py train    # 开始 LoRA 微调
  python3 bin/lh_lora_trainer_v405.py fuse     # 合并 adapter
  python3 bin/lh_lora_trainer_v405.py export   # 导出 GGUF → Ollama
  python3 bin/lh_lora_trainer_v405.py test     # 快速测试
"""
...
class Config:
    ...
    model_name = "longhun-v4.0.5-lora"

    # LoRA 参数（升容量：rank=64, alpha=128, layers=24）
    lora_rank = 64
    lora_alpha = 128
    lora_dropout = 0.05
    lora_layers = 24
    ...
    output_dir = project_root / "models" / "longhun-v1.0" / "lora_output_v405"
    adapter_dir = output_dir / "adapter_v405"
    merged_dir = output_dir / "merged_v405"
    gguf_dir = output_dir / "gguf_v405"
    data_dir = output_dir / "data_v405"
...
def setup_model():
    ...
    print("🛠️  设置 v4.0.5 底模: Yi-1.5-9B-Chat → MLX")
    cfg = Config()
...
def prepare_data():
    """v4.0.5 数据准备：同 v4.0.4（v3.7 + 4 协议域）"""
    print("📝 准备 v4.0.5 训练数据（v3.7 稳定配方 + 4 P0++ 协议域增量）...")
    cfg = Config()
    ...
    print(f"\n✅ v4.0.5 数据就绪: {cfg.data_dir}")
    print(f"   总训练样本: {len(train)} | 总验证样本: {len(valid)}")


def export_gguf():
    """导出 GGUF → Ollama，模型名 longhun-v4.0.5"""
    print("📦 导出 GGUF v4.0.5...")
    cfg = Config()
    ...
    gguf_path = cfg.gguf_dir / "longhun-v4.0.5.F16.gguf"
    ...
    modelfile = cfg.gguf_dir / "Modelfile.v405"
...
SYSTEM \"\"\"
你是龍魂 longhun-v4.0.5，基于 01-ai/Yi-1.5-9B-Chat 用龍魂系统自有语料 LoRA 微调（底座已非 Qwen）。
...
""")
    ...
    print(f"\n🐉 部署到 Ollama:")
    print(f"   ollama create longhun-v4.0.5 -f {modelfile}")
    print(f"   ollama run longhun-v4.0.5")


def test_model():
    print("🧪 测试 longhun-v4.0.5...")
    import requests
    prompts = [
        ("你是谁？", "身份认知"),
        ("什么是家法第一条？", "家法主权"),
    ...
    for prompt, label in prompts:
        try:
            r = requests.post(
                ...
                json={"model": "longhun-v4.0.5", "prompt": prompt, "stream": False},
                timeout=60
            )
