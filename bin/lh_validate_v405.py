...
#!/usr/bin/env python3
#龍芯⚡️丙午·乙未·甲寅·未时·乾-V405-VALIDATION-REPORT-v1.0
# CREATOR: 诸葛鑫 (UID9622)
# PROTOCOL: CC BY-NC-SA 4.0
# -*- coding: utf-8 -*-
"""
v4.0.5 部署验证脚本
DNA: #龍芯⚡️丙午·乙未·甲寅·未时·乾-V405-VALIDATION-REPORT-v1.0
"""
...
DATA_DIR = PROJECT / "models" / "longhun-v1.0" / "lora_output_v405" / "data_v405"
REPORT_DIR = PROJECT / "models" / "longhun-v1.0" / "lora_output_v405" / "validation_reports"
REPORT_DIR.mkdir(parents=True, exist_ok=True)
BEST_DIR = PROJECT / "models" / "longhun-v1.0" / "lora_output_v405" / "adapter_v4.0_best"
TRAINING_LOG = PROJECT / "models" / "longhun-v1.0" / "lora_output_v405" / "training.log"

OLLAMA_GENERATE_URL = "http://localhost:11434/api/generate"
OLLAMA_CHAT_URL = "http://localhost:11434/api/chat"
MODEL = "longhun-v4.0.5"
V37_BASELINE = 0.194
V392_BEST = 0.496
V403_BEST = 0.824
V404_BEST = 0.824
...
def load_v405_val_loss():
    # 优先读训练时写入的 best_val_loss.json
    loss_file = BEST_DIR / "val_loss.json"
    if loss_file.exists():
        try:
            return json.loads(loss_file.read_text(encoding='utf-8')).get("best_val_loss", V404_BEST)
        except Exception:
            pass
    # 兜底：从 training.log 解析
...
def val_loss_audit():
    print("📊 Val Loss 对比审计")
    v405_best = load_v405_val_loss()
    delta = V37_BASELINE - v405_best
    if v405_best < V37_BASELINE * 0.9:
        color = "🟢 进步"
    elif v405_best <= V37_BASELINE:
        color = "🟡 持平"
    else:
        color = "🔴 退步"
...
| 变化(v3.7→v4.0.5) | {loss_audit['delta']:+.4f} | **{loss_audit['color']}** |
...
| LoRA 容量 | rank=64, alpha=128, layers=24 |
"""
