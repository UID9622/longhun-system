    ...
    def hash(cls, data: Union[str, bytes]) -> bytes:
        ...
        for i in range(0, len(data), 64):
            ...
            for j in range(16, 68):
                w[j] = cls._p1(w[j - 16] ^ w[j - 9] ^ cls._rotate_left(w[j - 3], 15)) ^ cls._rotate_left(w[j - 13], 7) ^ w[j - 6]
...
class SM4:
    ...
    def _l(cls, a: int) -> int:
        return a ^ cls._rotate_left(a, 2) ^ cls._rotate_left(a, 10) ^ cls._rotate_left(a, 18) ^ cls._rotate_left(a, 24)
...
if __name__ == "__main__":
    # 自测
    print("SM3('abc') =", SM3.hex_hash("abc"))
    print("SM3 标准向量:", "66c7f0f462eeedd9d1f2d46bdc10e4e24167c4875cf2f7a2297da02b8f4ba8e0")

    key = b"0123456789abcdef"
    pt = b"hello CNSH guomi"
    ct = SM4.encrypt_ecb(pt, key)
    dt = SM4.decrypt_ecb(ct, key)
    print("SM4 解密正确:", dt == pt)
