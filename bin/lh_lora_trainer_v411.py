...
class Config:
    ...
    model_name = "longhun-v4.1.1-lora"

    # === LoRA 参数（dropout 0.15 更强抗过拟合） ===
    lora_rank = 16
    lora_alpha = 32
    lora_dropout = 0.15
    lora_layers = 12

    # === 训练参数（v4.1.1 调参：更低lr+更短warmup+更大batch） ===
    batch_size = 2
    ...
    output_dir = project_root / "models" / "longhun-v1.0" / "lora_output_v411"
    data_dir = output_dir / "data_v411_ready"
    adapter_dir = output_dir / "adapter_v411"
    merged_dir = output_dir / "merged_v411"
    gguf_dir = output_dir / "gguf_v411"
    ...
    if latest_v411:
        resume_adapter_file = latest_v411
...
def export_gguf():
    """导出 GGUF → Ollama，模型名 longhun-v4.1.1"""
    print("📦 导出 GGUF v4.1.1...")
    cfg = Config()

    if not (cfg.merged_dir / "config.json").exists():
        print(f"   ❌ 合并模型不存在: {cfg.merged_dir}")
        print(f"   请先运行: python3 bin/lh_lora_trainer_v411.py fuse")
        sys.exit(1)
    ...
    gguf_path = cfg.gguf_dir / "longhun-v4.1.1.F16.gguf"
    ...
    modelfile = cfg.gguf_dir / "Modelfile.v411"
...
SYSTEM \"\"\"
你是龍魂 longhun-v4.1.1，基于 01-ai/Yi-1.5-9B-Chat 用龍魂系统自有语料 LoRA 微调（底座已非 Qwen）。
...
"""
    ...
    print(f"\n🐉 部署到 Ollama:")
    print(f"   ollama create longhun-v4.1.1 -f {modelfile}")
    subprocess.run(
        ["ollama", "create", "longhun-v4.1.1", "-f", str(modelfile)], check=True
    )
    print(f"   ✅ Ollama 模型 longhun-v4.1.1 已创建")


def test_model():
    print("🧪 测试 longhun-v4.1.1...")
    import requests
    ...
    for prompt, label in prompts:
        try:
            r = requests.post(
                ...
                json={"model": "longhun-v4.1.1", "prompt": prompt, "stream": False},
                timeout=60,
            )
