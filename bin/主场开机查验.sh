...
echo ""
echo "【五道闸门 G1–G5】"
# G5: 简体龙 in key protocol files (sample)
if grep -rq '龙魂' "${LONGHUN_ROOT}/01_protocols" 2>/dev/null | grep -q '龍魂' 2>/dev/null; then
  echo "G5 字符律: 🟡 协议目录存在「龙/龍」混用，对外须人工审"
  log "gate" "G5 mixed 龙/龍 in protocols"
else
  echo "G5 字符律: 🟢 协议目录抽样通过"
  log "gate" "G5 protocol sample OK"
fi
