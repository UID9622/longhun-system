#!/usr/bin/env python3
#龍芯⚡️2026-07-20-MODEL-LORA-TRAINER-v4.1.2
# CREATOR: 诸葛鑫 (UID9622)
# PROTOCOL: CC BY-NC-SA 4.0
# -*- coding: utf-8 -*-
"""
🐉 龍魂 longhun-v4.1.2 LoRA 微调器（自定义训练循环 · 抗过拟合调参版 v4.1.2）
底模: 01-ai/Yi-1.5-9B-Chat (MLX) · 起点: v4.0.8-iter1900 golden (Val 0.767)
数据: v4.0.9 清洗集 → v4.1.2 工作目录
目标: v4.1.2 调参优化 - batch 4、alpha 64、warmup 100、dropout 0.12、
       epochs 3、patience 5 · 27,056条训练数据 · 从 v4.0.8 golden 恢复。

DNA: #龍芯⚡️2026-07-20-MODEL-LORA-TRAINER-v4.1.2

用法:
  python3 bin/lh_lora_trainer_v412.py setup    # 复用/转换 Yi-1.5-9B-Chat MLX
  python3 bin/lh_lora_trainer_v412.py prepare  # 从 v4.0.9_ready 复制数据到 v4.1.2_ready
  python3 bin/lh_lora_trainer_v412.py train    # 自定义 LoRA 训练循环
  python3 bin/lh_lora_trainer_v412.py fuse     # 合并 adapter
  python3 bin/lh_lora_trainer_v412.py export   # 导出 GGUF → Ollama
  python3 bin/lh_lora_trainer_v412.py test     # 快速测试

冒烟验证（5 iter，不污染正式训练）:
  LH_V411_SMOKE_ITERS=5 python3 bin/lh_lora_trainer_v412.py train
"""
...
def _dna():
    ...
    return f"#龍芯⚡️{ts}-MODEL-LORA-TRAINER-v4.1.2-{h}"
...
class Config:
    ...
    model_name = "longhun-v4.1.2-lora"
    ...
    # === 训练参数（v4.1.2 调参：batch 4·alpha 64·warmup 100） ===
    batch_size = 4
    ...
    output_dir = project_root / "models" / "longhun-v1.0" / "lora_output_v412"
    data_dir = project_root / "models" / "longhun-v1.0" / "lora_output_v411" / "data_v411_ready"
    adapter_dir = output_dir / "adapter_v412"
    merged_dir = output_dir / "merged_v412"
    gguf_dir = output_dir / "gguf_v412"
    ...
    if adapter_dir.exists():
        ...
        if numbered:
            latest_v412 = str(numbered[-1])
        elif (adapter_dir / "adapters.safetensors").exists():
            latest_v412 = str(adapter_dir / "adapters.safetensors")

    if latest_v412:
        resume_adapter_file = latest_v412
...
def setup_model():
    ...
    print("🛠️  设置 v4.1.2 底模: Yi-1.5-9B-Chat → MLX")
    cfg = Config()
...
def prepare_data():
    """从 data_v409_ready 复制数据到 data_v412_ready，后续可替换为 v4.1.2 增强数据"""
    print("📝 准备 v4.1.2 训练数据（从 v4.0.9 ready 复制）...")
    cfg = Config()
...
def _setup_logging(cfg: Config, smoke_mode: bool = False):
    ...
    if smoke_mode:
        import tempfile
        train_log_path = Path(tempfile.mktemp(prefix="v412_smoke_log_", suffix=".log"))
        log_mode = "w"
...
def _train_inner(cfg: Config):
    ...
    if not mlx_path.exists() or not list(mlx_path.rglob("*.safetensors")):
        ...
        print(f"   请先运行: python3 bin/lh_lora_trainer_v412.py setup")
        sys.exit(1)
    ...
    if not train_file.exists() or not valid_file.exists():
        ...
        print(f"   请先运行: python3 bin/lh_lora_trainer_v412.py prepare")
        sys.exit(1)
    ...
    if smoke_iters:
        smoke_mode = True
        import tempfile
        cfg.adapter_dir = Path(tempfile.mkdtemp(prefix="v412_smoke_"))
        print(
    ...
    if smoke_mode:
        ...
        print(f"   下一步: python3 bin/lh_lora_trainer_v412.py fuse")


def fuse():
    """合并 LoRA adapter → 完整模型"""
    print("🔗 合并 LoRA adapter v4.1.2...")
    cfg = Config()

    adapter_cfg = cfg.adapter_dir / "adapter_config.json"
    if not adapter_cfg.exists():
        print(f"   ❌ Adapter 不存在: {cfg.adapter_dir}")
        print(f"   请先运行: python3 bin/lh_lora_trainer_v412.py train")
        sys.exit(1)
    ...
    print(f"   下一步: python3 bin/lh_lora_trainer_v412.py export")


def export_gguf():
    """导出 GGUF → Ollama，模型名 longhun-v4.1.2"""
    print("📦 导出 GGUF v4.1.2...")
    cfg = Config()

    if not (cfg.merged_dir / "config.json").exists():
        print(f"   ❌ 合并模型不存在: {cfg.merged_dir}")
        print(f"   请先运行: python3 bin/lh_lora_trainer_v412.py fuse")
        sys.exit(1)
    ...
    gguf_path = cfg.gguf_dir / "longhun-v4.1.2.F16.gguf"
    ...
    modelfile = cfg.gguf_dir / "Modelfile.v412"
...
SYSTEM \"\"\"
你是龍魂 longhun-v4.1.2，基于 01-ai/Yi-1.5-9B-Chat 用龍魂系统自有语料 LoRA 微调（底座已非 Qwen）。
...
"""
    ...
    print(f"\n🐉 部署到 Ollama:")
    print(f"   ollama create longhun-v4.1.2 -f {modelfile}")
    subprocess.run(
        ["ollama", "create", "longhun-v4.1.2", "-f", str(modelfile)], check=True
    )
    print(f"   ✅ Ollama 模型 longhun-v4.1.2 已创建")


def test_model():
    print("🧪 测试 longhun-v4.1.2...")
    import requests
    ...
    for prompt, label in prompts:
        try:
            r = requests.post(
                ...
                json={"model": "longhun-v4.1.2", "prompt": prompt, "stream": False},
                timeout=60,
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or sys.argv[1] not in commands:
        ...
        print(f"  {'prepare':12} → 从 v4.0.9_ready 复制数据到 v4.1.2_ready")
