...
class NamingRegistry:
    ...
    def register(self, path: str, cn_comment: str) -> dict:
    ...
    def deprecate(self, old_path: str, new_path: str = "", compat_days: int = 90) -> dict:
        """废弃流程：冻结不删除·兼容期90天"""
...
class TaijiEnvelope:
    ...
    def create(trace_id: str = "", api_path: str = "", version: int = 1,
               ...
               gate: str = "#CONFIRM🌌9622-ONLY-ONCE🧬LK9X-772Z") -> dict:
        """创建完整太极封套"""
        ts = int(time.time())
        m_segment = {
    ...
    @staticmethod
    def compute_entropy(envelope: dict) -> dict:
        """计算封套信息熵 (bit)"""
...
class AntColonyScheduler:
    ...
    def ant_can_execute(self, ant_role: str, action: str) -> dict:
        """蚁群守则验证"""
    ...
    def assign_task(self, task_id: str, force_ant: str = "") -> dict:
        """任务分派"""
        if force_ant:
...
class PersonaRouter:
    ...
    def anti_cosplay_check(self, output: str) -> dict:
    ...
    def route(self, persona_code: str, query: str = "") -> dict:
        """人格路由"""
        now = time.time()
...
class MultiLevelRateLimiter:
    ...
    def check(self, palace: str, method: str, ip: str = "0.0.0.0") -> dict:
        """三维限流检查 · 任一维度桶空 → 🟡限流"""
...
class VersionNegotiator:
    ...
    def negotiate(self, client_version: str, client_prefers: List[str] = None) -> dict:
        """版本协商"""
...
def error_response(error_code: str, detail: str = "", request_id: str = "") -> dict:
    """标准错误响应"""
...
class APIGateway:
    ...
    def process_request(self, path: str, method: str, headers: dict = None,
                        cn_comment: str = "", body: dict = None) -> dict:
    ...
    def _validate_path(self, path: str, cn_comment: str) -> dict:
        """路径主权校验"""
        # 中文路径 🔴
    ...
    def auto_register_core(self) -> dict:
        """自动注册核心八宫基础接口"""
        results = []
