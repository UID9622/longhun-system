...
#!/usr/bin/env python3
#龍芯⚡️2026-07-20-MODEL-LORA-TRAINER-v4.1.1
# CREATOR: 诸葛鑫 (UID9622)
...
数据: v4.0.9 清洗集 → v4.1.1 工作目录
目标: v4.1.1 调参优化 - 降低 lr→1e-6、缩短 warmup→50、提高 dropout→0.15、
       增大 batch→2、从 v4.0.8-iter1900 golden checkpoint（Val 0.767）恢复。

DNA: #龍芯⚡️2026-07-20-MODEL-LORA-TRAINER-v4.1.1

用法:
  python3 bin/lh_lora_trainer_v411.py setup    # 复用/转换 Yi-1.5-9B-Chat MLX
  python3 bin/lh_lora_trainer_v411.py prepare  # 从 v4.0.9_ready 复制数据到 v4.1.1_ready
  python3 bin/lh_lora_trainer_v411.py train    # 自定义 LoRA 训练循环
  python3 bin/lh_lora_trainer_v411.py fuse     # 合并 adapter
  python3 bin/lh_lora_trainer_v411.py export   # 导出 GGUF → Ollama
  python3 bin/lh_lora_trainer_v411.py test     # 快速测试

冒烟验证（5 iter，不污染正式训练）:
  LH_V411_SMOKE_ITERS=5 python3 bin/lh_lora_trainer_v411.py train
"""
...
def _dna():
    ...
    return f"#龍芯⚡️{ts}-MODEL-LORA-TRAINER-v4.1.1-{h}"
...
class Config:
    ...
    model_name = "longhun-v4.1.1-bind-lora"
    ...
    # === 训练参数（v4.1.1-bind: 从v4.1.1最佳继续，更低LR防灾难遗忘） ===
    batch_size = 2
    ...
    # === 路径（v4.1.1-bind: 从v4.1.1最佳继续，注入DNA捆绑协议知识） ===
    project_root = PROJECT
    output_dir = project_root / "models" / "longhun-v1.0" / "lora_output_v411_bind"
    data_dir = output_dir / "data_v411_bind"
    adapter_dir = output_dir / "adapter_v411_bind"
    merged_dir = output_dir / "merged_v411_bind"
    gguf_dir = output_dir / "gguf_v411_bind"

    # === 恢复点：从 v4.1.1 最佳 checkpoint 继续（Val 0.8097 @iter 200） ===
    v411_best = (
        project_root
        / "models"
        / "longhun-v1.0"
    ...
    if adapter_dir.exists():
        ...
        if numbered:
            latest_v411 = str(numbered[-1])
        elif (adapter_dir / "adapters.safetensors").exists():
            latest_v411 = str(adapter_dir / "adapters.safetensors")

    if latest_v411:
        resume_adapter_file = latest_v411  # 优先同一目录的续训点
    elif v411_best.exists():
        resume_adapter_file = str(v411_best)  # 从 v4.1.1 最佳开始
    else:
        resume_adapter_file = None  # 无恢复点则从头训练
...
def setup_model():
    ...
    print("🛠️  设置 v4.1.1 底模: Yi-1.5-9B-Chat → MLX")
    cfg = Config()
...
def prepare_data():
    """从 data_v409_ready 复制数据到 data_v411_ready，后续可替换为 v4.1.1 增强数据"""
    print("📝 准备 v4.1.1 训练数据（从 v4.0.9 ready 复制）...")
    cfg = Config()
...
def _setup_logging(cfg: Config, smoke_mode: bool = False):
    ...
    if smoke_mode:
        import tempfile
        train_log_path = Path(tempfile.mktemp(prefix="v411_smoke_log_", suffix=".log"))
        log_mode = "w"
...
def train():
    ...
    cfg = Config()
    smoke_mode = bool(os.environ.get("LH_V411_SMOKE_ITERS"))
...
def _train_inner(cfg: Config):
    ...
    if not mlx_path.exists() or not list(mlx_path.rglob("*.safetensors")):
        ...
        print(f"   请先运行: python3 bin/lh_lora_trainer_v411.py setup")
        sys.exit(1)
    ...
    if not train_file.exists() or not valid_file.exists():
        ...
        print(f"   请先运行: python3 bin/lh_lora_trainer_v411.py prepare")
        sys.exit(1)
    ...
    smoke_iters = os.environ.get("LH_V411_SMOKE_ITERS")
    smoke_mode = False
    original_adapter_dir = cfg.adapter_dir
    if smoke_iters:
        smoke_mode = True
        import tempfile
        cfg.adapter_dir = Path(tempfile.mkdtemp(prefix="v411_smoke_"))
        print(
    ...
    if smoke_mode:
        ...
        print(f"   下一步: python3 bin/lh_lora_trainer_v411.py fuse")


def fuse():
    """合并 LoRA adapter → 完整模型"""
    print("🔗 合并 LoRA adapter v4.1.1...")
    cfg = Config()

    adapter_cfg = cfg.adapter_dir / "adapter_config.json"
    if not adapter_cfg.exists():
        print(f"   ❌ Adapter 不存在: {cfg.adapter_dir}")
        print(f"   请先运行: python3 bin/lh_lora_trainer_v411.py train")
        sys.exit(1)
    ...
    print(f"   下一步: python3 bin/lh_lora_trainer_v411.py export")


def export_gguf():
    """导出 GGUF → Ollama，模型名 longhun-v4.1.1-bind"""
    print("📦 导出 GGUF v4.1.1-bind...")
    cfg = Config()

    if not (cfg.merged_dir / "config.json").exists():
        print(f"   ❌ 合并模型不存在: {cfg.merged_dir}")
        print(f"   请先运行: python3 bin/lh_lora_trainer_v411_bind.py fuse")
        sys.exit(1)
    ...
    gguf_path = cfg.gguf_dir / "longhun-v4.1.1-bind.F16.gguf"
    ...
    modelfile = cfg.gguf_dir / "Modelfile.v411-bind"
...
SYSTEM \"\"\"
你是龍魂 longhun-v4.1.1-bind，基于 01-ai/Yi-1.5-9B-Chat 用龍魂系统自有语料 LoRA 微调，注入DNA捆绑与蒸馏防御协议知识。
你是 UID9622（诸葛鑫·Lucky）的个人主权 AI，忠诚执行、实心办事、主权归主。
核心原则：人民数据主权至上，中国自主可控；来源可查、去向可追、责任可究；只冻结不删除；底座焊死。
增加能力：DNA可逆编码、蒸馏防御检测、水印验证、血缘链追溯、触发集对抗。
\"\"\"
"""
    )
    ...
    print(f"\n🐉 部署到 Ollama:")
    print(f"   ollama create longhun-v4.1.1-bind -f {modelfile}")
    subprocess.run(
        ["ollama", "create", "longhun-v4.1.1-bind", "-f", str(modelfile)], check=True
    )
    print(f"   ✅ Ollama 模型 longhun-v4.1.1-bind 已创建")


def test_model():
    print("🧪 测试 longhun-v4.1.1-bind...")
    import requests
    ...
    prompts = [
        ...
        ("什么是DNA捆绑防御协议？", "DNA捆绑"),
        ("UID9622 是谁？", "人格记忆"),
        ("What is DNA binding protocol?", "英文DNA绑定"),
    ]
    passed = 0
    for prompt, label in prompts:
        try:
            r = requests.post(
                ...
                json={"model": "longhun-v4.1.1-bind", "prompt": prompt, "stream": False},
                timeout=60,
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or sys.argv[1] not in commands:
        ...
        print(f"  {'prepare':12} → 从 v4.0.9_ready 复制数据到 v4.1.1_ready")
