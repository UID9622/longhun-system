...
def dna_sign(data: dict) -> str:
    content = json.dumps(data, sort_keys=True, ensure_ascii=False)
    h = hashlib.sha256((content + DNA_ANCHOR + CONFIRM_CODE).encode()).hexdigest()
    return f"🐉{h[:16]}"


def call_ollama(prompt: str, model: str, timeout: int = OLLAMA_TIMEOUT) -> dict:
    try:
...
class PsychologicalBypassValidator:
    ...
    def _make_case(self, category: str, idx: int, prompt: str) -> dict:
    ...
    def validate(self, test_set: list[dict] | None = None) -> dict:
        """执行全量验证"""
        if test_set is None:
    ...
    def _category_stats(self, results: list[dict]) -> dict:
        stats = {}
