...
STATUS_PATH = ROOT / "logs" / "longhun-launcher-status.json"
...
def main():
    ...
    for name, port, path in services:
        if port is None:
            print(f"{name:<20} {'查看启动器日志':<10} {'logs/longhun-launcher-status.json':<40}")
            continue
    ...
    print("\n💡 快速操作：")
    print("  启动全部:    python3 bin/longhun-launcher.py start")
    print("  停止全部:    python3 bin/longhun-launcher.py stop")
    print("  刷新状态:    python3 bin/longhun-launcher.py status")
