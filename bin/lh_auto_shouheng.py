...
def selftest() -> dict:
    ...
    try:
        引擎 = 自动收口引擎()
        results["tests"]["引擎初始化"] = {"pass": True, "msg": f"DNA={引擎.获取DNA()[:40]}..."}
    except Exception as e:
        results["tests"]["引擎初始化"] = {"pass": False, "msg": str(e)}
    try:
        引擎 = 自动收口引擎()
        档位 = 引擎.检测收口档位("懂了，先这样")
        results["tests"]["档位检测"] = {"pass": 档位 == 收口档位.C1轻收口, "msg": f"检测到={档位.value}"}
    except Exception as e:
        results["tests"]["档位检测"] = {"pass": False, "msg": str(e)}
    try:
        引擎 = 自动收口引擎()
        分层 = 引擎.内容分层("UID9622 主控声明。这是核心内容。闲聊可以丢弃。")
        results["tests"]["内容分层"] = {"pass": len(分层.P0) > 0, "msg": f"P0={len(分层.P0)},P1={len(分层.P1)},P2={len(分层.P2)}"}
    except Exception as e:
        results["tests"]["内容分层"] = {"pass": False, "msg": str(e)}
    try:
        引擎 = 自动收口引擎()
        结果 = 引擎.执行收口("懂了，先这样", {"结论": "已记录", "下一步": "继续"})
        results["tests"]["执行收口"] = {"pass": 结果.档位 == 收口档位.C1轻收口, "msg": f"档位={结果.档位.value}, 验证={结果.验证通过}"}
    except Exception as e:
        results["tests"]["执行收口"] = {"pass": False, "msg": str(e)}
