...
# ═══════════════════════════════════════════
# ENCODING: UTF-8
# DNA追溯码：#龍芯⚡️2026-07-06-CNSH-GATEKEEPER-v1.0
...
龍_DNA_正则 = re.compile(r'#龍芯⚡️\d{4}-\d{2}-\d{2}-[A-Z0-9\-]+', re.IGNORECASE)
...
class CNSH闸门:
    ...
    def 检查Python文件(self, 路径: Path) -> 闸门结果:
        ...
        # 4. 繁简归一
        if '龙魂' in 内容 and '龍魂' not in 内容:
            结果.添加警告("繁简归一", "发现简体'龙魂'，应使用繁体'龍魂'")
