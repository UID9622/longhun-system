...
def _老祖宗规则审计写入(文件路径: str, 层级: str, 触发词: str, 操作: str, 来源: str = "cnsh_gatekeeper"):
    """独立审计日志·与主权覆写审计物理分离"""
    entry = {
...
class CNSH闸门:
    ...
    def 检查Python文件(self, 路径: Path) -> 闸门结果:
        ...
        是闸门自身 = 路径.name == 'cnsh_gatekeeper.py'
        ...
        if not 是闸门自身:
            ...
            if 老祖宗_红命中:
                ...
                _老祖宗规则审计写入(str(路径), "L1-L4", 触発詞, "BLOCK", "cnsh_gatekeeper")
            elif 老祖宗_变体命中:
                触発詞 = 老祖宗_变体命中[0]
                结果.添加拒绝(
                    f"🧬 老祖宗规则锚定·🔴变体熔断",
                ...
                _老祖宗规则审计写入(str(路径), "L3-变体", 触発詞, "BLOCK", "cnsh_gatekeeper")
            elif 老祖宗_黄命中:
                结果.添加警告(
                    f"🧬 老祖宗规则锚定·🟡预警",
                    f"需追问上下文：{', '.join(老祖宗_黄命中[:5])}"
                )
                _老祖宗规则审计写入(str(路径), "黄色预警", 老祖宗_黄命中[0], "WARN", "cnsh_gatekeeper")
            else:
                结果.添加通过("🧬 老祖宗规则锚定 - 通过")
        else:
