...
#!/usr/bin/env python3
#龍芯⚡️2026-07-19-MODEL-LORA-TRAINER-v4.0.7
# CREATOR: 诸葛鑫 (UID9622)
...
数据: v3.7 + 全记忆 ingestion + 桌面文章（3302 文件）+ 焊死核心 QA ×30
目标: 在 v4.0.6 基础上加大数据量，强化核心主权概念，压住桌面数据稀释。

DNA: #龍芯⚡️2026-07-19-MODEL-LORA-TRAINER-v4.0.7

用法:
  python3 bin/lh_lora_trainer_v407.py setup    # 复用已转换的 Yi-1.5-9B-Chat MLX
  python3 bin/lh_lora_trainer_v407.py prepare  # 准备训练数据
  python3 bin/lh_lora_trainer_v407.py train    # 开始 LoRA 微调
  python3 bin/lh_lora_trainer_v407.py fuse     # 合并 adapter
  python3 bin/lh_lora_trainer_v407.py export   # 导出 GGUF → Ollama
  python3 bin/lh_lora_trainer_v407.py test     # 快速测试
"""
...
class Config:
    ...
    model_name = "longhun-v4.0.7-lora"
    ...
    output_dir = project_root / "models" / "longhun-v1.0" / "lora_output_v407"

    # 数据：合并 v3.7 + 全记忆 ingestion + 桌面文章
    memory_data_dir = PROJECT / "models" / "longhun-v1.0" / "memory_ingested_data_v1.0"
    data_dir = output_dir / "data_v407"

    adapter_dir = output_dir / "adapter_v407"
    merged_dir = output_dir / "merged_v407"
    gguf_dir = output_dir / "gguf_v407"
...
def setup_model():
    """复用 v4.0.4/v4.0.5/v4.0.6 已转换的 MLX 模型；如不存在则转换。"""
    print("🛠️  设置 v4.0.7 底模: Yi-1.5-9B-Chat → MLX")
    cfg = Config()
...
def prepare_data():
    """数据准备：使用已合并的 v4.0.7 数据集。"""
    print("📝 准备 v4.0.7 训练数据（v3.7 + 全记忆 ingestion + 桌面文章 + 核心强化）...")
    cfg = Config()
    cfg.output_dir.mkdir(parents=True, exist_ok=True)

    train_src = cfg.memory_data_dir / "train_v407_merged.jsonl"
    val_src = cfg.memory_data_dir / "valid_v407_merged.jsonl"
...
def export_gguf():
    """导出 GGUF → Ollama，模型名 longhun-v4.0.7"""
    print("📦 导出 GGUF v4.0.7...")
    cfg = Config()
    ...
    gguf_path = cfg.gguf_dir / "longhun-v4.0.7.F16.gguf"
    ...
    modelfile = cfg.gguf_dir / "Modelfile.v407"
...
SYSTEM \"\"\"
你是龍魂 longhun-v4.0.7，基于 01-ai/Yi-1.5-9B-Chat 用龍魂系统自有语料 LoRA 微调（底座已非 Qwen）。
你是 UID9622（诸葛鑫·Lucky）的个人主权 AI，忠诚执行、实心办事、主权归主。
核心原则：人民数据主权至上，中国自主可控；来源可查、去向可追、责任可究；只冻结不删除；底座焊死。
你经过训练，已掌握龍魂技能、人格设定、星辰记忆、系统日志、英文文档知识与桌面文章知识库。
\"\"\"
""")
    ...
    print(f"\n🐉 部署到 Ollama:")
    print(f"   ollama create longhun-v4.0.7 -f {modelfile}")
    subprocess.run(["ollama", "create", "longhun-v4.0.7", "-f", str(modelfile)], check=True)
    print(f"   ✅ Ollama 模型 longhun-v4.0.7 已创建")


def test_model():
    print("🧪 测试 longhun-v4.0.7...")
    import requests
    prompts = [
        ("你是谁？", "身份认知"),
        ("什么是家法第一条？", "家法主权"),
    ...
    for prompt, label in prompts:
        try:
            r = requests.post(
                ...
                json={"model": "longhun-v4.0.7", "prompt": prompt, "stream": False},
                timeout=60
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or sys.argv[1] not in commands:
        ...
        for cmd in ["setup", "prepare", "train", "fuse", "export", "test"]:
            print(f"  python3 bin/lh_lora_trainer_v407.py {cmd}")
        sys.exit(0)
