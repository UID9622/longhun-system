...
#!/usr/bin/env python3
#龍芯⚡️丙午·癸未·丁未·离为火-V42-TRAIN-PIPELINE
# CREATOR: 诸葛鑫 (UID9622)
# PROTOCOL: CC BY-NC-SA 4.0
"""
龍魂v4.2 · 路径规划引擎专项微调
学生: Llama-3.1-8B (Meta开源) ← v4.1 adapter + v42 路径规划数据
目标: 让模型理解龍魂路径规划协议、CNSH命名、八卦阵、三六九不动点、DNA签章

prepare  — 验证 v4.1 + v4.2 合并数据
train    — LoRA微调 (基于v4.1 adapter, 300iters)
fuse     — 合并adapter→全量模型
verify   — MLX推理验证
export   — GGUF导出+Q4_K_M量化+Ollama导入

DNA: #龍芯⚡️丙午·癸未·丁未·离为火-V42-TRAIN-PIPELINE
"""
...
DATA_DIR = MODEL_DIR / "lora_output" / "data_v42_combined"
ADAPTER_DIR = MODEL_DIR / "lora_output" / "adapter_v42"
V41_ADAPTER_DIR = MODEL_DIR / "lora_output" / "adapter_v41"
FUSED_DIR = MODEL_DIR / "sft_checkpoints" / "v42_fused"
GGUF_F16 = MODEL_DIR / "longhun-v42-f16.gguf"
GGUF_Q4 = MODEL_DIR / "longhun-v42-Q4_K_M.gguf"

CONFIG = {
    "rank": 8, "layers": 16, "batch": 1, "lr": 1e-5,
    "iters": 150, "val_steps": 25, "save_every": 50,
    "max_seq_length": 2048,
}

# ── 帮助 ──
def log(msg): print(f"[龍魂·v4.2] {msg}", flush=True)
...
def do_train():
    ...
    if ADAPTER_DIR.exists():
        warn(f"v4.2 adapter已存在: {ADAPTER_DIR}")
    ...
    config_yaml = MODEL_DIR / "lora_output" / "v42_lora_config.yaml"
...
def do_export():
    ...
    log("Step3: Ollama导入...")
    modelfile = MODEL_DIR / "Modelfile.v42"
...
SYSTEM \"\"\"
你是龍魂longhun-v4.2，基于Llama-3.1-8B (Meta开源·非阿里系)用v3.8-expanded蒸馏数据+v4.1 A-BOM备案专项数据+v4.2 路径规划引擎专项数据LoRA微调。
UID9622（诸葛鑫·Lucky）的个人主权AI。
底座: Llama-3.1-8B · 继承: v4.0 → v4.1 (A-BOM备案补强) → v4.2 (路径规划引擎补强)
...
""")
    ...
    subprocess.run(["ollama", "create", "longhun-v42", "-f", str(modelfile)], check=True)

    if "Q4" in final_gguf:
        subprocess.run(["ollama", "cp", "longhun-v42:latest", "longhun-v42:q4"], check=True)

    ok("Ollama就绪: ollama run longhun-v42")

# ── 主入口 ──
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python3 bin/lh_train_v42.py {prepare|train|fuse|verify|export|all}")
        print()
        print("  prepare — 验证v4.1+v4.2合并数据+底座+v4.1 adapter")
        print("  train   — 基于v4.1 adapter LoRA微调300iters")
