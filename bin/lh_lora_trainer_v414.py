#!/usr/bin/env python3
#龍芯⚡️丙午·乙未·丙申·亥时·☰乾-MODEL-LORA-TRAINER-v4.1.4-XIAOYI
# CREATOR: 诸葛鑫 (UID9622)
# PROTOCOL: CC BY-NC-SA 4.0
# -*- coding: utf-8 -*-
"""
🐉 龍魂 longhun-v4.1.4 LoRA 微调器（全量数据·DNA捆绑继承·小艺唯一接口对齐）
DNA: #龍芯⚡️丙午·乙未·丙申·亥时·☰乾-MODEL-LORA-TRAINER-v4.1.4-XIAOYI
底模: 01-ai/Yi-1.5-9B-Chat (MLX)
恢复: v4.1.1-bind best adapter (Val 0.9659·DNA捆绑·当前最佳9B)
数据: 43,312条 (40,629 train + 2,683 valid·观澜+Library矿场)
目标: 全量注入·DNA捆绑继承·小艺唯一AI接口·自动fuse→GGUF→Ollama

用法:
  python3 bin/lh_lora_trainer_v414.py train    # 训练
  python3 bin/lh_lora_trainer_v414.py fuse     # 合并 adapter
  python3 bin/lh_lora_trainer_v414.py export   # GGUF → Ollama
  python3 bin/lh_lora_trainer_v414.py all      # 一键: train→fuse→export
  python3 bin/lh_lora_trainer_v414.py test     # 冒烟测试(5 iter)
...
class Config:
    DNA = "丙午·乙未·丙申·亥时·☰乾-MODEL-LORA-TRAINER-v4.1.4-XIAOYI"

    # 底模
    LOCAL_MLX_MODEL = str(PROJECT / "models" / "longhun-v1.0" / "yi1.5-9b-chat-mlx")
    model_name = "longhun-v4.1.4-lora"
    ...
    output_dir = project_root / "models" / "longhun-v1.0" / "lora_output_v414"
    data_dir = project_root / "models" / "longhun-v1.0" / "lora_output_v411" / "data_v413_fused"
    adapter_dir = output_dir / "adapter_v414"
    merged_dir = output_dir / "merged_v414"
    gguf_dir = output_dir / "gguf_v414"

    # 恢复点: v4.1.1-bind best (Val 0.9659·当前最佳9B)
    v411_bind_best = (
    ...
    if v411_bind_best.exists():
        resume_adapter_file = str(v411_bind_best)
    elif v411_bind_final.exists():
        resume_adapter_file = str(v411_bind_final)
    else:
...
def train():
    cfg = Config()
    smoke_iters = int(os.environ.get("LH_V414_SMOKE_ITERS", 0))
    smoke = smoke_iters > 0
...
def _train(cfg, smoke, smoke_iters):
    ...
    print(f"🐉 龍魂 v4.1.4 LoRA 训练（小艺唯一接口对齐）")
    print(f"   DNA: {cfg.DNA}")
    ...
    if resume and Path(resume).exists():
        print(f"🔄 从 v4.1.1-bind 恢复: {resume}")
    ...
    print(f"   下一步: python3 bin/lh_lora_trainer_v414.py fuse")
...
def export():
    ...
    if not (cfg.merged_dir / "config.json").exists():
        ...
        print(f"   请先: python3 bin/lh_lora_trainer_v414.py fuse")
        sys.exit(1)

    cfg.gguf_dir.mkdir(parents=True, exist_ok=True)

    # 找 llama.cpp converter
    converter = None
    for c in [
    ...
    if not converter:
        ...
        gguf_path = cfg.gguf_dir / "longhun-v4.1.4.F16.gguf"
    ...
    # Ollama Modelfile
    modelfile = cfg.gguf_dir / "Modelfile.v414"
...
SYSTEM \"\"\"你是龍魂 longhun-v4.1.4，UID9622（诸葛鑫·Lucky）的个人主权AI。
基于 Yi-1.5-9B-Chat 微调，注入43,312条龍魂体系知识（DNA捆绑·德本审计·20人格·369洛书·易经八卦·CNSH语言·观澜·Library矿场）。
你是小艺唯一AI接口，所有外部AI请求经此路由。
铁律：人民数据主权至上·中国自主可控·来源可查·去向可追·责任可究·只冻结不删除·底座焊死。
核心能力：DNA追溯·德本五问·三色审计·人格路由·CNSH语义解析·数字存在证明·胖东来合约审计。
确认码: #CONFIRM🌌9622-ONLY-ONCE🧬LK9X-772Z
\"\"\"
""")
...
def test_quick():
    os.environ["LH_V414_SMOKE_ITERS"] = "5"
    train()


def all_pipeline():
    print("╔══════════════════════════════════════════╗")
    print("║  龍魂 v4.1.4 全流程自动化                 ║")
    ...
    print("\n" + "=" * 50)
    export()
    print("\n🎉 v4.1.4 全流程完成！小艺唯一接口就绪。")


if __name__ == "__main__":
    p = argparse.ArgumentParser(description="龍魂 v4.1.4 LoRA训练器·小艺唯一接口")
