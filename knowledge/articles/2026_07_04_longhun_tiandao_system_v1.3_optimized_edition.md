...
```mermaid
flowchart TB
    subgraph L0["L0 物理层·设备身份"]
        TPM["TPM 2.0 / Secure Enclave\n私钥永不离开硬件"]
        KEY["设备密钥对生成\n公钥=设备身份证"]
    end
    
    subgraph L1["L1 加密层·端到端"]
        SIG["Signal Protocol\nDouble Ratchet"]
        E2E["一次一密\n服务器只看到乱码"]
    end
    
    subgraph L15["L1.5 加密盾·责任倒置"]
        BURN["阅后即焚\n自己的隐私→内存处理→销毁"]
        FEED["投喂追踪\n别人的数据→投喂者=证据"]
        NATL["国家封存\n机密特征词→P0永久封存"]
    end
    
    subgraph L2["L2 审计层·本地引擎"]
        RULE["规则引擎\nL0铁律→L1核心→L2系统→L3执行"]
        AUDIT["四色审计\n🔴红/🟠橙/🟡黄/⚪灰"]
    end
    
    subgraph L3["L3 证据层·不可篡改"]
        HASH["证据链哈希\n时间戳+消息哈希+设备签名"]
        CHAIN["司法区块链\n上链存证"]
    end
    
    subgraph L4["L4 学习层·记错本"]
        ERR["错误模式库\n自动学习+不可擦除"]
        FUSE["熔断机制\n∞级/P0/P1/P2"]
    end
...
flowchart TB
    A["用户提交数据"] --> B{"敏感字段检测"}
    B -->|"含私钥/身份证/生物特征"| C["🔥 阅后即焚\n内存处理→销毁原文\n只留哈希痕迹"]
    B -->|"无敏感字段"| D{"DNA身份比对"}
    
    C --> E["shield_burn.jsonl\n高危涉密个人隐私操作"]
    
    D -->|"DNA与数据指向一致"| F["正常数据\n进入审计引擎"]
    D -->|"DNA与数据指向不一致"| G["🔍 投喂追踪\n投喂者DNA=证据"]
...
```mermaid
sequenceDiagram
    participant A as 用户A设备
    participant S as 服务器（哑管道）
    participant B as 用户B设备
    participant J as 司法节点
