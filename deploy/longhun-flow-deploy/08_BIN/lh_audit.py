...
# DNA: #龍芯⚡️丙午·丙申·己未·乙亥时·䷒临-AUDIT-v1.0-UID9622
...
def _four_pillars(now: datetime) -> str:
    ...
    修正: gua_of_day 已统一为梅花易数签名 gua_of_day(date, hour) → (id, name, symbol)"""
    if _HAS_GANZHI:
        dg, _didx = _dz(now.date())
        _gid, _gname, _gsym = _gua(now.date(), now.hour)
        return f"{_yz(now.date())}·{_mz(now.date())}·{dg}·{_hz(now.hour, dg[0])}时·{_gsym}{_gname}"
