...
> 后端三服务（8970/19622/18800）仅绑定 `127.0.0.1`，绕过 nginx 直连视为违规，自动上耻辱墙。

---

## 🔥 鲲鹏生产增量部署记录 (2026-08-14)

**部署结果**: ✅ 已上线（增量适配，未覆盖生产）

| 项 | 值 |
|:---|:---|
| 服务 | `longhun-flow-api`(:8970) / `longhun-flow-collab`(:19622) / `longhun-flow-bridge`(:18800) |
| 状态 | 全部 active + enabled + NRestarts=0 |
| 健康 | `/health/flow` → 200「龍魂流量拓扑运行正常」 |
| 鉴权 | `/auth/verify` 带真实 DNA → 200 `{"auth":"ok"}`；无 DNA → 401（P0 DNA 契约生效） |
| 路由 | `/chat` `/collab/api/` `/handoffs/` `/protocols/` 已合并进主站 443 server 块 |
| 备份 | `/var/backups/longhun/flow-incremental-20260814_091954/` |

**适配改动（相对原交付包）**:
1. **端口**: 对话桥接 18799 → 18800（18799 被 frps 内网穿透占用）
2. **systemd 单元名**: `longhun-api/collab/bridge` → `longhun-flow-*`（避免覆盖生产 `longhun-api.service`）
3. **nginx**: 不覆盖主配置（zone/upstream/server_name 与生产冲突），改为:
   - conf.d 增量片段 `longhun-flow-incremental.conf`（zone 用 `flow_*` 前缀、upstream 用 `flow_longhun_*`）
   - 增量 location 精确合并进 uid9622.cn 主站 443 server 块（62-439行），不碰 api.uid9622.cn
4. **未覆盖**: nginx.conf / /var/www/longhun/index.html / /etc/logrotate.d/longhun / certbot.timer
5. **生产既有问题（部署前已存在）**: `/`(9627)、`/portal/`(8789) 502 是生产旧服务未启动，与本次部署无关；`longhun-api.service` activating(auto-restart) 为历史遗留

**回滚**: `sudo /opt/longhun-flow-deploy/rollback.sh`（备份在 /var/backups/longhun/）

> 原 deploy.sh 仍适用于全新服务器一键部署；鲲鹏生产用 `deploy-flow-incremental.sh`。
