...
SUB_DISPATCH = {
    ...
    # 🚨 龍魂 12 孤儿引擎接线 v1.0 — 造了就要用·lh 一键直达（2026-08-28 补接线·U001）
    'agent-exec':           ('../05_ENGINES/lh_agent_executor.py', '🤖', 'Agent执行器·submit/type/persona/execute/list/status/dry-run', ['--list']),
    'entropy':              ('../05_ENGINES/lh_ai_entropy_calibrator.py', '🌀', '反熵增引擎v2.0·真实AI熵采集与参数重标定(自检+标定)', []),
    'entropy-run':          ('../05_ENGINES/lh_ai_entropy_calibrator.py', '⚡', '反熵增·全量标定(耗时)', []),
    'entropy-monte':        ('../05_ENGINES/lh_ai_entropy_engine.py', '🎲', '反熵增v1.0·10万次Monte Carlo推演(攻坚·耗时)', []),
    'car-cloud':            ('../05_ENGINES/lh_car_cloud_index.py', '🚗', '龍魂车载·云端索引服务v2.1(零依赖·LONGHUN_CAR_PORT改端口)', ['--help']),
    'car-edge':             ('../05_ENGINES/lh_car_edge_index.py', '🗺️', '龍魂车载·边缘实景导航索引v1.1(零依赖·LONGHUN_CAR_PORT改端口)', ['--help']),
    'clip-vault':           ('../05_ENGINES/lh_clipboard_vault.py', '🗂️', '剪贴板容器·save/list/kg-ready', ['list']),
    'fengshui':             ('../05_ENGINES/lh_fengshui_game_engine.py', '🏮', '风水场博弈引擎v1.0·直接运行推演测试', []),
    'mem-archive':          ('../05_ENGINES/lh_fixed_point_memory_archive.py', '🏛️', '不动点记忆归档·ingest/status/search/pending/quarantine', ['status']),
    'gov-api':              ('../05_ENGINES/lh_governance_api.py', '🏛️', '行业痛点治理API·--host/--port', ['--help']),
    'gov-industry':         ('../05_ENGINES/lh_industry_governance.py', '🏭', '行业痛点治理编排·assess/act/dashboard', ['dashboard']),
    'writer':               ('../05_ENGINES/lh_terminal_writer.py', '✍️', '多模型终端写作·ask/auto/config/status/stats', ['status']),
    'video-agent':          ('../05_ENGINES/lh_video_agent.py', '🎬', '视频创作智能体·--topic/--style/--materials', []),
