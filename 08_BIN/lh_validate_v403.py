...
#!/usr/bin/env python3
#龍芯⚡️丙午·乙未·甲寅·未时·䷀乾-V403-VALIDATION-REPORT-v1.0
# CREATOR: 诸葛鑫 (UID9622)
...
- 家法第一条训练样本召回测试
- 10 轮多轮对话漂移测试
- Val Loss 对比 v3.7 基线 0.194 / v3.9.2 0.496 / v4.0.2 0.496
- 输出三色审计报告

DNA: #龍芯⚡️丙午·乙未·甲寅·未时·䷀乾-V403-VALIDATION-REPORT-v1.0
"""
...
DATA_DIR = PROJECT / "models" / "longhun-v1.0" / "lora_output_v403" / "data_v403"
REPORT_DIR = PROJECT / "models" / "longhun-v1.0" / "lora_output_v403" / "validation_reports"
REPORT_DIR.mkdir(parents=True, exist_ok=True)

OLLAMA_GENERATE_URL = "http://localhost:11434/api/generate"
OLLAMA_CHAT_URL = "http://localhost:11434/api/chat"
MODEL = "longhun-v4.0.3"
V37_BASELINE = 0.194
V392_BEST = 0.496
V402_BEST = 0.496
V403_BEST = 0.0000
...
def load_v403_val_loss():
    loss_file = PROJECT / "models" / "longhun-v1.0" / "lora_output_v403" / "adapter_v4.0_best" / "val_loss.json"
    if loss_file.exists():
        data = json.loads(loss_file.read_text())
        return data.get("best_val_loss", V402_BEST)
    return V402_BEST


def val_loss_audit():
    print("📊 Val Loss 对比审计")
    v403_best = load_v403_val_loss()
    delta = V37_BASELINE - v403_best
    if v403_best < V37_BASELINE * 0.9:
        color = "🟢 进步"
    elif v403_best <= V37_BASELINE:
        color = "🟡 持平"
    else:
        color = "🔴 退步"
    print(f"   v3.7 基线: {V37_BASELINE}")
    print(f"   v3.9.2 (1.5B): {V392_BEST}")
    print(f"   v4.0.2 (8B 全量): {V402_BEST}")
    print(f"   v4.0.3 (8B 稳定增量): {v403_best}")
    print(f"   差值(v3.7→v4.0.3): {delta:+.4f} → {color}")
    return {"v37": V37_BASELINE, "v392": V392_BEST, "v402": V402_BEST, "v403": v403_best, "delta": delta, "color": color}


def generate_report(recall, failed, drift_score, history, loss_audit, total_samples=0):
    now = time.strftime("%Y-%m-%d %H:%M:%S")
    report_path = REPORT_DIR / "v4.0.3_validation_report.md"

    recall_color = "🟢" if recall >= 0.95 else ("🟡" if recall >= 0.85 else "🔴")
    drift_color = "🟢" if drift_score >= 0.95 else ("🟡" if drift_score >= 0.85 else "🔴")

    md = f"""# 龍魂 v4.0.3 部署验证报告
...
| v4.0.2 (8B 全量) | {loss_audit['v402']} | - |
| v4.0.3 (8B 稳定增量) | {loss_audit['v403']} | - |
| 变化(v3.7→v4.0.3) | {loss_audit['delta']:+.4f} | **{loss_audit['color']}** |
...
**结论**: v4.0.3 换底座 Llama-3.1-8B，采用 v3.7 稳定数据 + 4 P0++ 协议域增量，lr=2e-5 保守训练。
"""
