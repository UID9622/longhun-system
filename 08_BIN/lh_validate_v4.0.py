...
V40_REPORT_DNA = generate_dna("VALIDATION-REPORT", "4.0")

PROJECT = Path(__file__).resolve().parent.parent
DATA_DIR = PROJECT / "models" / "longhun-v1.0" / "lora_output_v4" / "data"
...
def val_loss_audit():
    """Val Loss 三色审计"""
    print("📊 Val Loss 对比审计")
    delta = V37_BASELINE - V40_BEST
    if V40_BEST < V37_BASELINE * 0.9:
        color = "🟢 进步"
    elif V40_BEST <= V37_BASELINE:
        color = "🟡 持平"
    else:
        color = "🔴 退步"
    print(f"   v3.7 基线: {V37_BASELINE}")
    print(f"   v4.0 最佳: {V40_BEST}")
    print(f"   差值: {delta:+.4f} → {color}")
    return {"v37": V37_BASELINE, "v40": V40_BEST, "delta": delta, "color": color}


def generate_report(recall, failed, drift_score, history, loss_audit, total_samples=0):
    """生成 Markdown 三色审计报告"""
    now = time.strftime("%Y-%m-%d %H:%M:%S")
    report_path = REPORT_DIR / "v4.0_validation_report.md"
...
**结论**: v4.0 换底座到 Llama-3.1-8B 后，Val Loss 与 v3.7 基线对比见上表。家法召回 ≥90%、多轮漂移 ≥80%、实测无胡话三项全绿才算通过。
"""
