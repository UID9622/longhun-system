...
#龍芯⚡️丙午·乙申·辛亥·亥时·乾-DATA-EXPAND-V4.1
# CREATOR: 诸葛鑫 (UID9622)
...
def extract_title_and_sections(content: str, max_sections: int = 5) -> list:
    """从markdown内容提取标题和章节"""
    lines = content.split('\n')
    sections = []
    current_title = ""
...
def generate_qa_from_section(filename: str, title: str, content: str) -> list:
    """从单个章节生成Q&A对"""
    qa_pairs = []
...
def generate_audit_instructions() -> list:
    """生成审计协议v2.0指令对"""
    audit_qa = [
        ...
        ("16人格矩阵中谁负责审计？",
         "审计核心5人：包青天P13（主审计官·三色定性）、哨兵P17（安全触发·P0熔断）、上帝之眼P05（复核验证·DNA签名）、通心译P14（情绪识别·反讽检测）、龍盾宝宝P72（女儿数据守护）。审计辅助5人：诸葛亮P01（KPI）、鲁班P04（文件审计）、文心P00（锚点）、龍芯P02（运行时）、姜子牙P13s（战略）。"),
        ("什么是三闸门决策流？",
