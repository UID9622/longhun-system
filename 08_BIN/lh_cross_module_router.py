...
class CrossModuleRouter:
    ...
    def get_paper(self, name: str) -> dict:
        """获取论文注册信息"""
        return self.registry["papers"].get(name, {})

    def list_engines_by_status(self, status: str = None) -> list:
        """按状态列出引擎"""
    ...
    def list_papers_without_engines(self) -> list:
        """列出有数学但没引擎的论文"""
    ...
    def list_protocols_without_engines(self) -> list:
        """列出有数学但没引擎的协议"""
    ...
    def audit_citation_chain(self) -> dict:
        """全系统引用链审计·返回三色报告"""
        report = {"🟢": [], "🟡": [], "🔴": [], "summary": {}}
    ...
    def dispatch(self, source_engine: str, event: str, payload: dict = None, depth: int = 0) -> dict:
        """派发回调事件·含熔断保护"""
    ...
    def _invoke_engine(self, target: str, method: str, payload: dict, eng_info: dict) -> str:
        """动态调用目标引擎方法"""
