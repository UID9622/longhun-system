...
def bits_to_gua(bits: tuple) -> str:
    """二进制数组→卦象"""
...
def state_to_hexagram(bits: tuple) -> dict:
    """状态向量→六十四卦信息"""
    idx = bits_to_int(bits)
...
def yao_bian(state: tuple, position: int) -> tuple:
...
def gua_bian(state: tuple, mask: tuple) -> tuple:
    """卦变（多比特翻转）: T_v(G) = G ⊕ v"""
...
def hamiltonian_path(steps: int = 64) -> list:
    """生成卦变哈密顿路径（最小变化走到目标）
...
def gua_entropy(probs: list) -> float:
    """卦爻熵 H = -Σ p_i log₂ p_i
...
def yao_entropy(yao_probs: list) -> float:
    """单爻熵"""
...
def sancai_flow_field(state: tuple) -> dict:
    """三才流场分解
...
def sancai_energy(flow: dict) -> float:
    """三才能量计算
...
def world_state_energy(state: tuple) -> dict:
    """世界状态能级分析"""
...
class YijingWorldEngine:
    ...
    def get_state(self, idx_or_bits) -> dict:
        """获取指定状态"""
    ...
    def get_bagua(self, name: str) -> dict:
        """获取八卦定义"""
        bits = BAGUA.get(name)
        if bits is None:
    ...
    def evolve(self, from_state, to_state) -> dict:
    ...
    def sancai_decompose(self, state) -> dict:
        """三才分解"""
        bits = state if isinstance(state, tuple) else self.get_state(state)["bits"]
