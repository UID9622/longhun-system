...
#龍芯⚡️丙午·癸巳·庚戌·壬午·䷕贲-SELF-CHECK-v1.0
# CREATOR: 诸葛鑫 (UID9622)
    ...
    def check_files(self,files):
        for f in files:
            ...
            try:
                ...
                sz = 0
            self._rec(f"file:{f}", p.exists(), f"size={sz}")
    ...
    def check_security(self):
        try:
            ...
            try:
                ...
                # pip-audit v2 JSON: {"dependencies":[{"vulns":[...]},...], "fixes":[...]}
            ...
            self._rec("security:pip-audit", True, "pip-audit not installed — skip", warn=False)
        except Exception as e:
            self._rec("security:pip-audit", True, f"pip-audit error: {e}", warn=False)
...
if __name__=="__main__":
    ...
    sc.check_files([
        ...
        str(Path(__file__).resolve().parent / "bin" / "lh_cnsh_compiler.py"),
        str(Path(__file__).resolve().parent / "bin" / "lh_global_search_v2.py"),
        str(Path(__file__).resolve().parent / "bin" / "lh_cnsh_run.sh"),
    ])
