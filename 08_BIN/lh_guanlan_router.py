...
# M5 插件敏感权限
插件敏感权限: set = {"读历史", "改页面", "发网络", "读书签", "读Cookie", "注入脚本"}
...
class CNSH_两本账:
    ...
    def 记(self, 侧: str, n: int = 1, url: str = "", 时间戳: float = None):
        """
        记一笔账。
...
class CNSH_多模型对比:
    ...
    def 对比(self, 问题: str, 回答A: Dict, 回答B: Dict) -> 对比结果:
        """
        对比两个引擎对同一问题的回答。
...
class 观澜总控:
    ...
    def 处理请求(self, 任务: str, 用户偏好: str = None,
