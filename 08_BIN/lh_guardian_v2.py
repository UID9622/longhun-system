...
#!/usr/bin/env python3
#龍芯⚡️丙午·乙未·丙戌·甲午·䷕贲-GUARDIAN-v2.0-LK9X-772Z
# CREATOR: 诸葛鑫 (UID9622)
...
DNA: #龍芯⚡️丙午·乙未·丙戌·甲午·䷕贲-GUARDIAN-v2.0-LK9X-772Z
...
class 守護配置:
    ...
    # --- 預編譯黑名單正則（整詞邊界匹配·防偽裝繞過） ---
    _挖礦黑名單正則: Optional[re.Pattern] = field(default=None, init=False)

    def __post_init__(self):
        """預編譯挖礦黑名單正則 + 可疑端口字符串集緩存"""
        if self.挖礦黑名單:
            pattern = r'\b(?:' + '|'.join(re.escape(kw) for kw in self.挖礦黑名單) + r')\b'
            self._挖礦黑名單正則 = re.compile(pattern, re.IGNORECASE)
    ...
    動態白名單最低巡邏次數: int = 20  # 需要20輪巡邏才能進入動態白名單 (約100秒觀察)
    動態白名單威脅上限: float = 15.0  # 威脅評分 <15 才能進入 (原20太寬鬆)
    ...
    # --- DNA標識 ---
    系統UID: str = "UID9622"
    DNA標記: str = "#龍芯⚡️丙午·乙未·丙戌·甲午·䷕贲-GUARDIAN-v2.0"
    確認碼: str = "LK9X-772Z"
...
class 龍魂憲兵隊:
    ...
    def __init__(self, 配置: Optional[守護配置] = None):
        ...
        # === 哈希鏈 — 前向鏈接，確保每條日誌不可單獨篡改 ===
        self._上一哈希: str = ""  # 上一条审计日志的哈希，形成链
    ...
    def _審計日誌(self, 事件類型: str, 內容: str, 級別: 安全級別, 元數據: Optional[Dict] = None):
        ...
        🔗 哈希鏈: 每條記錄包含上一條記錄的哈希，形成不可篡改鏈
        符合 lh_governance 三色審計規範
        """
        審計條目 = {
    ...
    def _第一道防線_黑名單檢測(self, name: str, cmdline: str) -> Tuple[bool, Optional[str]]:
        ...
        🔒 使用正則整詞邊界匹配 (\b)，防止偽裝繞過
          例如: "syslogd-xmrig" → 不匹配 (避免誤殺含關鍵詞的正常進程)
                "xmrig" 獨立詞 → 🔴 熔斷
        返回: (是否通過, 匹配到的關鍵詞)
        """
        ...
        # 正則整詞匹配（優先：防偽裝繞過）
        if self.配置._挖礦黑名單正則:
            match = self.配置._挖礦黑名單正則.search(full_text)
            if match:
    ...
    def _第四道防線_行為分析(self, 畫像: 進程行為畫像) -> 安全級別:
        """
        第四道防線: 行為畫像分析 — 多維加權綜合判定
        🔥 v2.1 加固: 威脅評分作為主信號 + 黑色熔斷路徑
        
        判定邏輯:
          1. 威脅評分 >= 90 且連接礦池端口 → ⚫ BLACK  (焦土級)
          2. 威脅評分 >= 80 → 🔴 RED    (立即處決)
          3. 威脅評分 >= 50 + 持續異常 → 🔴 RED
          4. 威脅評分 >= 30 或 CPU/MEM超黃線 → 🟡 YELLOW (告警+限速)
          5. 威脅評分 < 30 → 🟢 GREEN
        ...
        # 檢查礦池端口連接（挖礦鐵證）
        # 網路端點格式為 "ip:port"，提取端口號比對可疑端口列表
        可疑端口集合 = {str(p) for p in self.配置.可疑端口}
        可疑連接 = 0
        for ep in 畫像.網路端點:
            if ":" in ep:
                port_str = ep.rsplit(":", 1)[1]
                if port_str in 可疑端口集合:
                    可疑連接 += 1
        ...
        # === 紅色: 高威脅評分 ===
        if 威脅 >= 80 or (威脅 >= 55 and 畫像.黃色持續次數 >= self.配置.黃色持續次數觸發紅):
            畫像.黃色持續次數 += 1
        ...
        # === 黃色: 威脅中或單維超標 ===
        if (威脅 >= 30 or 
            cpu_latest > self.配置.CPU閾值_黃 or 
            mem_latest > self.配置.記憶體閾值_黃 or
            可疑連接 > 0):
            畫像.黃色持續次數 += 1
            if 畫像.黃色持續次數 >= self.配置.黃色持續次數觸發紅:
                return 安全級別.紅_嚴重
    ...
    def _熔斷_立即處決(self, pid, name, 畫像, 原因):
        """紅色: SIGTERM → 等待子進程退出 → SIGKILL 物理消滅（連坐子進程）"""
        try:
            子進程列表 = self._獲取子進程(pid)

            # 先給優雅退出機會
            os.kill(pid, signal.SIGTERM)
            for child_pid in 子進程列表:
                try:
                    os.kill(child_pid, signal.SIGTERM)
                except Exception:
                    pass

            # 等待足夠時間讓進程清理資源 (v2.1: 0.5s → 2s)
            time.sleep(2.0)
            ...
            for child_pid in 子進程列表:
                try:
                    if psutil.pid_exists(child_pid):
                        ...
                        self._審計日誌("KILL_CHILD", f"🔴 子進程連坐 PID:{child_pid} (父:{pid})", 安全級別.紅_嚴重, {
                            "parent_pid": pid, "child_pid": child_pid, "parent_name": name
                        })
                except Exception:
                    pass
    ...
    def _獲取子進程(self, parent_pid: int) -> List[int]:
        """獲取指定進程的所有子進程PID（用於連坐處決）"""
        child_pids = []
        try:
            ...
            try:
                ...
                if self._第二道防線_靜態白名單(name, cmdline):
                    ...
                    if 級別 in (安全級別.黃_警告, 安全級別.紅_嚴重):
                        ...
                        # 行為正常 → 動態白名單（三條件: 威脅低 + 足夠觀察 + 無違規記錄）
                        if (畫像.威脅評分 < self.配置.動態白名單威脅上限 and 
                            self.巡邏次數 >= self.配置.動態白名單最低巡邏次數 and
                            畫像.黃色持續次數 == 0):
                            self.動態白名單.add(指紋)
                    continue
...
if __name__ == "__main__":
    ...
    if args.status:
        ...
        print(_json.dumps(憲兵隊.獲取狀態報告(), indent=2, ensure_ascii=False))
        sys.exit(0)
