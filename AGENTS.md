...
## 6.10 AI 产出幻觉量化自检（2026-09-05 焊死 · 启动必载 · 常驻 · 所有 AI 对准）
- **铁律**：龍魂体系所有 AI 交付（报告/文章/协议/规则/对外发言）前过幻觉量化自检——**实机跑过才标「已验证」，没跑过只标「推演」，无出处自称事实 = 🔴 禁止交付**（判据④）。
- **工具**：`lh halluc`（`08_BIN/lh_hallucination_metrics.py`·H 综合指数三色 🟢≥0.80/🟡≥0.50/🔴<0.50·seed=9622 验收可复现·独立数学验证器·ECE 单独汇报）。
- **硬触发**：产出 ≥10 句实质论断 → 自检三问（每条论断有出处？实测还是推演？会不会是我编的？），有 gold 数据跑 `lh halluc` 量化。
- **制度**：完整条款 = 对齐规则第二十一层（`.codebuddy/rules/longhun-codebuddy-alignment-v2.md` v2.9·alwaysApply 注入每个 AI 会话）；老大焊点原话见 21.5。
