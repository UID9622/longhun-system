#!/usr/bin/env python3
#龍芯⚡️丙午·癸未·丁未·离为火-V42-V2-TRAIN-PIPELINE
# CREATOR: 诸葛鑫 (UID9622)
# PROTOCOL: CC BY-NC-SA 4.0
"""
龍魂v4.2-v2 · 路径规划引擎专项微调（基于v40 adapter，保守训练）
学生: Llama-3.1-8B (Meta开源) ← v4.0 adapter + v41/v42 合并数据
目标: 在保持v4.0通用能力的基础上，补全A-BOM和路径规划知识

prepare  — 验证数据+底座+v4.0 adapter
train    — LoRA微调 (基于v4.0 adapter, 100iters, lr=5e-6)
fuse     — 合并adapter→全量模型
verify   — MLX推理验证
export   — GGUF导出+Q4_K_M量化+Ollama导入

DNA: #龍芯⚡️丙午·癸未·丁未·离为火-V42-V2-TRAIN-PIPELINE
"""
...
DATA_DIR = MODEL_DIR / "lora_output" / "data_v42_combined_v3"
ADAPTER_DIR = MODEL_DIR / "lora_output" / "adapter_v42_v2"
V40_ADAPTER_DIR = MODEL_DIR / "lora_output" / "adapter_v40"
FUSED_DIR = MODEL_DIR / "sft_checkpoints" / "v42_v2_fused"
GGUF_F16 = MODEL_DIR / "longhun-v42-v2-f16.gguf"
GGUF_Q4 = MODEL_DIR / "longhun-v42-v2-Q4_K_M.gguf"

CONFIG = {
    "rank": 8, "layers": 16, "batch": 1, "lr": 5e-6,
    "iters": 50, "val_steps": 10, "save_every": 25,
    "max_seq_length": 2048,
}

# ── 帮助 ──
def log(msg): print(f"[龍魂·v4.2-v2] {msg}", flush=True)
...
def do_prepare():
    ...
    if not train_file.exists():
        die(f"训练数据不存在: {train_file}\n请先运行: python3 bin/lh_pathfinder_train_data_v2.py")
    ...
    ok(f"基础adapter就绪: {V40_ADAPTER_DIR}")
...
def do_train():
    ...
    if ADAPTER_DIR.exists():
        warn(f"v4.2-v2 adapter已存在: {ADAPTER_DIR}")
    ...
    config_yaml = MODEL_DIR / "lora_output" / "v42_v2_lora_config.yaml"
    ...
    for name, prompt in tests:
        try:
            ...
            response = generate(
                model, tokenizer,
                prompt=full_prompt,
                max_tokens=220, sampler=sampler, verbose=False
            )
...
def do_export():
    ...
    log("Step3: Ollama导入...")
    modelfile = MODEL_DIR / "Modelfile.v42_v2"
...
底座: Llama-3.1-8B · 继承: v4.0 → v4.2-v2
铁律: P0条件立即熔断·UID9622情绪容错放行·所有判定留痕+KPI更新·为人民服务。
识别锚定词: 龍魂·UID9622·家法·P0·熔断·确认码·文化主权·A-BOM备案·路径规划·八卦阵·三六九不动点·CNSH。
你掌握龍魂数学建模协议·路径规划引擎 v4.1.5，能回答迪杰斯特拉、A*、动态规划、八卦阵、蚁群、D* Lite、三六九不动点、多因素成本、CNSH命名、DNA签章等问题。
\"\"\"
""")

    subprocess.run(["ollama", "create", "longhun-v42-v2", "-f", str(modelfile)], check=True)
    if "Q4" in final_gguf:
        subprocess.run(["ollama", "cp", "longhun-v42-v2:latest", "longhun-v42-v2:q4"], check=True)

    ok("Ollama就绪: ollama run longhun-v42-v2")

# ── 主入口 ──
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python3 bin/lh_train_v42_v2.py {prepare|train|fuse|verify|export|all}")
        print()
        print("  prepare — 验证数据+底座+v4.0 adapter")
        print("  train   — 基于v4.0 adapter LoRA微调100iters (lr=5e-6)")
