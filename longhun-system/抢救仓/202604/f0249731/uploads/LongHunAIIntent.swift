...
struct SanSeAuditIntent: AppIntent {
    ...
    static var description = IntentDescription("龙魂L2审计系统，返回🟢🟡🔴")
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> {
        guard let url = URL(string: "http://192.168.100.253:8765/三色审计") else {
            return .result(value: "❌ 无法连接到龙魂本地服务")
        }
...
struct GenerateDNAIntent: AppIntent {
    ...
    static var description = IntentDescription("生成龙魂DNA追溯码")
    ...
    @MainActor
    func perform() async throws -> some IntentResult & ReturnsValue<String> {
        guard let url = URL(string: "http://192.168.100.253:8765/生成DNA") else {
            return .result(value: "❌ 无法连接到龙魂本地服务")
        }
...
struct QueryLongHunStatusIntent: AppIntent {
    static var title: LocalizedStringResource = "查询龙魂状态"
    static var description = IntentDescription("查询龙魂系统运行状态")
    
    @MainActor
    func perform() async throws -> some IntentResult & ReturnsValue<String> {
        guard let url = URL(string: "http://192.168.100.253:8765/查询状态") else {
            return .result(value: "❌ 无法连接到龙魂本地服务")
        }
        ...
        do {
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                let 系统 = json["系统名称"] as? String ?? "龙魂系统"
...
struct AskLongHunIntent: AppIntent {
    static var title: LocalizedStringResource = "问龙魂"
    static var description = IntentDescription("与龙魂本地AI对话，100%数据主权")
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> {
        // 调用本地模型API
        guard let url = URL(string: "http://192.168.100.253:8765/对话") else {
            return .result(value: "❌ 无法连接到龙魂本地服务")
        }
...
struct SearchMemoryIntent: AppIntent {
    static var title: LocalizedStringResource = "搜索龙魂记忆"
    ...
    @MainActor
    func perform() async throws -> some IntentResult & ReturnsValue<String> {
        guard let url = URL(string: "http://192.168.100.253:8765/查询记忆?关键词=\(keyword.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")") else {
            return .result(value: "❌ 无法连接到龙魂本地服务")
        }
...
struct SaveMemoryIntent: AppIntent {
    static var title: LocalizedStringResource = "保存到龙魂"
    ...
    @MainActor
    func perform() async throws -> some IntentResult & ReturnsValue<String> {
        guard let url = URL(string: "http://192.168.100.253:8765/保存记忆") else {
            return .result(value: "❌ 无法连接到龙魂本地服务")
        }
...
struct QueryNotionIntent: AppIntent {
    static var title: LocalizedStringResource = "查询龙魂知识库"
    static var description = IntentDescription("搜索Notion知识库，龙魂大脑")
    ...
    @MainActor
    func perform() async throws -> some IntentResult & ReturnsValue<String> {
        guard let url = URL(string: "http://192.168.100.253:8765/查询Notion") else {
            return .result(value: "❌ 无法连接到龙魂本地服务")
        }
        ...
        let body = ["关键词": query] as [String: Any]
        request.httpBody = try? JSONSerialization.data(withJSONObject: body)
        ...
        do {
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                let 状态 = json["状态"] as? String ?? ""
                let 结果数 = json["结果数"] as? Int ?? 0
                let 页面列表 = json["页面列表"] as? [[String: Any]] ?? []
                
                if 状态 == "成功" && !页面列表.isEmpty {
                    var 报告 = "找到 \(结果数) 个相关页面：\n"
                    
                    for (i, 页面) in 页面列表.prefix(5).enumerated() {
                        let 标题 = 页面["标题"] as? String ?? "无标题"
                        报告 += "\n\(i + 1). \(标题)"
                    }
                    ...
                    return .result(value: 报告)
                } else if let 错误 = json["错误"] as? String {
                    return .result(value: "❌ \(错误)")
                } else {
