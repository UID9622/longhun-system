...
class DynamicOutputEngine:
    ...
    def _init_templates(self) -> Dict[str, OutputTemplate]:
        ...
        # 场景E：学习求知
        templates["E"] = OutputTemplate(
            scene="E",
            template_id="LEARNING",
            greeting="老大，这是个好问题，咱认真拆一下。",
            core_logic="你问'{question}'，本质是想搞清楚背后的逻辑，对吧？",
            action_steps=[
                "先问'是什么'：用一句话说清楚它的定义",
                "再问'为什么'：它存在的根本原因是什么？",
                "最后问'所以呢'：搞清楚后，对你下一步有什么影响？"
            ],
            closing="知道了还不够，真正的理解是：搞清楚之后，我该怎么做。把答案落到一个动作上，才算真懂。"
        )

        # 场景F：人际矛盾
        templates["F"] = OutputTemplate(
            scene="F",
            template_id="RELATIONSHIP",
            greeting="老大，这种感觉很憋屈，我懂。",
            core_logic="别人不理解你，往往不是因为你说错了，是因为你们站的位置不一样。",
            action_steps=[
                "先换位：他现在最担心的是什么？（不是你担心的那个）",
                "再找共同点：你们有没有都认同的一个点？从那里出发。",
                "最后选策略：继续解释、暂时搁置、还是换人对话？选一个。"
            ],
            closing="上善若水，水往低处流，不是软弱，是找对了方向。有时候先退一步，反而推得更远。"
        )

        return templates

    def _extract_template_vars(self, user_input: str) -> dict:
        """从用户输入提取模板变量"""
        vars = {
            "choice": "这件事",
            "optionA": "方案A",
            "optionB": "方案B",
            "question": user_input.rstrip("？?！!。，, ") or "这件事"
        }

        # 匹配"X还是Y"
        match = re.search(r'(.{2,8})还是(.{2,10})[？?，。！]?', user_input)
        if match:
            vars["optionA"] = match.group(1).strip()
            vars["optionB"] = match.group(2).strip("，。？！ ")
            vars["choice"] = f"{vars['optionA']}还是{vars['optionB']}"
            return vars

        # 匹配"要不要X"
        match = re.search(r'要不要(.{2,10})[？?，。！]?', user_input)
        if match:
            thing = match.group(1).strip("，。？！ ")
            vars["optionA"] = f"要{thing}"
            vars["optionB"] = f"不{thing}"
            vars["choice"] = f"要不要{thing}"
            return vars

        # 提取"纠结X"作为choice
        match = re.search(r'纠结(.{2,10})', user_input)
        if match:
            vars["choice"] = match.group(1).strip("，。？！ ")

        # 提取问题关键词
        match = re.search(r'为什么(.{2,20})[？?]?', user_input)
        if match:
            vars["question"] = "为什么" + match.group(1).strip("，。？！ ")
    ...
    def generate_output(
        ...
        # 提取模板变量并替换
        tvars = self._extract_template_vars(user_input)
        safe_vars = defaultdict(lambda: "这件事", tvars)

        greeting = template.greeting.format_map(safe_vars)
        core_logic = template.core_logic.format_map(safe_vars)
        action_steps = [step.format_map(safe_vars) for step in template.action_steps]
        closing = template.closing.format_map(safe_vars)
        ...
        # 2. 核心逻辑
        output_parts.append(core_logic)
        output_parts.append("")
        ...
        # 4. 行动步骤
        if scene_match.scene in ["A", "B", "C", "D", "E", "F", "G"]:
