...
class AILibraryLogRequest(BaseModel):
    ai_name: str        # 例如：Claude / Kimi / ChatGPT
    action: str         # 例如：模块提交 / DNA生成 / 代码审计
...
def get_library():
    ...
    conn = db.get_conn()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM ai_library ORDER BY logged_at DESC LIMIT 200')
    rows = cursor.fetchall()
    conn.close()
    ...
    for log in logs:
        ...
        if name not in summary:
            summary[name] = {'count': 0, 'actions': []}
        summary[name]['count'] += 1
        if log['action'] not in summary[name]['actions']:
            summary[name]['actions'].append(log['action'])

    return {
        "title": "龍魂AI图书馆 · 有借有还 · 无人看管 · 主打自觉",
        "owner": "诸葛鑫（UID9622）",
        "dna": "#龍芯⚡️2026-03-23-LIBRARY-UID9622",
        "total_logs": len(logs),
        "ai_summary": summary,
        "logs": logs
    }

@app.post("/library/log")
def log_library(request: AILibraryLogRequest):
    """AI图书馆 - 登记一次AI操作"""
    conn = db.get_conn()
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO ai_library (ai_name, action, dna, detail)
        VALUES (?, ?, ?, ?)
    ''', (request.ai_name, request.action, request.dna, request.detail))
    conn.commit()
    ...
    return {
        ...
        "dna": request.dna,
        "message": f"{request.ai_name} 的操作已留痕"
    }
