...
### ☯️ 三才算法统一内核 `core/sancai_kernel.py`
...
>>> from sancai_kernel import sancai_check
>>> sancai_check("龍魂系统", "内核测试", 2026)
{'color': '🟡', '天': DR=3, '地': 六维路径+不动点, '人': 上下文分析}
```

- **数字根**：`DR(n) = 1 + ((n-1) % 9)` → 三色熔断（🟢🟡🔴）
- **五行生克**：木→火→土→金→水循环 · 平衡度分析
- **不动点网络**：13个锚点（龍/道德经/曾仕强...），任何变换下不变
- **洛书矩阵**：3×3验证（行/列/对角线和=15）
- **六十甲子**：完整甲子→癸亥60位表

### 🧠 记忆压缩引擎 `core/memory_compress.py`

记忆压缩 ≈ 量子计算 ≈ 时空压缩 — 底层数学完全一致：

```
P0 核心记忆 → 100%保留（不动点命中 = 锚不能动）
P1 重要记忆 → 60%语义保留（家人/情感/决策）
P2 日常记忆 → 30%关键词（工作/学习）
P3 临时记忆 → 10%摘要指纹（一般问答）

100年记忆 ≈ 0.1 GB · 一个U盘装得下
```

- 数字根+不动点联合分级（避免日常文本误判）
- 五行语义平衡检查（压缩后偏差>0.4自动补偿）
- 复用三才内核全部数学基础

### 🌸 通心译翻译伦理引擎 `docs/tongxinyi_ethics_engine.md`

翻译关系，不翻译文字。CNSH编辑器的温柔表达层：

- **主权词汇锁**：龍/五行/八卦 永不翻译
- **解释型熔断**：拦截时解释为什么，不是冷冰冰地拒绝
- **方言支持**：温州话/粤语/东北话/四川话/闽南语/客家话/吴语
- **语气词保真**：嘛/呢/哈/嘞/咯 — 语气也是语义
