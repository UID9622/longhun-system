  ...
  get event() {
    this._event ??= (callback, thisArgs, disposables, extension) => {
      ...
      if (_listenerFinalizers) {
        ...
        const match2 = /(file:|vscode-file:\/\/vscode-app)?(\/[^:]*:\d+:\d+)/.exec(stack2);
        _listenerFinalizers.register(result, match2?.[2] ?? stack2, result);
      }
      return result;
    };
    return this._event;
...
// out-build/vs/base/common/worker/simpleWorkerBootstrap.js
...
var LocalFileSearchSimpleWorkerHost = class _LocalFileSearchSimpleWorkerHost {
  static {
    this.CHANNEL_NAME = "localFileSearchWorkerHost";
  }
  static getChannel(workerServer) {
    return workerServer.getChannel(_LocalFileSearchSimpleWorkerHost.CHANNEL_NAME);
  }
  static setChannel(workerClient, obj) {
    workerClient.setChannel(_LocalFileSearchSimpleWorkerHost.CHANNEL_NAME, obj);
  }
};
...
var getFileResults = (bytes, pattern, options) => {
  let text;
...
// out-build/vs/workbench/services/search/common/ignoreFile.js
var IgnoreFile = class {
  ...
  gitignoreLinesToExpression(lines, dirPath, trimForExclusions2) {
    ...
    return parse2(includeExpression, { trimForExclusions: trimForExclusions2 });
  }
...
// out-build/vs/workbench/services/search/worker/localFileSearch.js
var PERF = false;
var globalStart = +/* @__PURE__ */ new Date();
var itrcount = {};
var time = async (name, task) => {
  if (!PERF) {
    return task();
  }
...
var LocalFileSearchSimpleWorker = class {
  ...
  async $listDirectory(handle, query, folderQuery, ignorePathCasing, queryId) {
    ...
    const extUri2 = new ExtUri(() => ignorePathCasing);
    ...
    await time("listDirectory", () => this.walkFolderQuery(handle, reviveQueryProps(query), revivedFolderQuery, extUri2, (file) => {
    ...
    return time("searchInFiles", async () => {
      ...
      const processFile = async (file) => {
        ...
        if (fileResults.length) {
          ...
          this.host.$sendTextSearchMatch(match2, queryId);
          results.push(match2);
        }
      };
      await time("walkFolderToResolve", () => this.walkFolderQuery(handle, reviveQueryProps(query), revivedQuery, extUri2, async (file) => onGoingProcesses.push(processFile(file)), token.token));
  ...
  async walkFolderQuery(handle, queryProps, folderQuery, extUri2, onFile, token) {
    const folderExcludes = folderQuery.excludePattern?.map((excludePattern) => parse2(excludePattern.pattern ?? {}, { trimForExclusions: true }));
    const evalFolderExcludes = (path, basename3, hasSibling) => {
      return folderExcludes?.some((folderExclude) => {
        return folderExclude(path, basename3, hasSibling);
      });
    };
    const isFolderExcluded = (path, basename3, hasSibling) => {
      path = path.slice(1);
      if (evalFolderExcludes(path, basename3, hasSibling)) {
        return true;
      }
    ...
    const isFileIncluded = (path, basename3, hasSibling) => {
      path = path.slice(1);
      if (evalFolderExcludes(path, basename3, hasSibling)) {
        return false;
      }
      if (!pathIncludedInQuery(queryProps, path, extUri2)) {
        return false;
      }
      return true;
    };
    ...
    const processDirectory = async (directory, prior, ignoreFile) => {
      ...
      const entries = Promises.withAsyncBody(async (c) => {
        ...
        for (const [basename3, handle2] of entries2) {
          if (token.isCancellationRequested) {
            break;
          }
          const path = prior + basename3;
          if (ignoreFile && !ignoreFile.isPathIncludedInTraversal(path, handle2.kind === "directory")) {
            continue;
          }
          const hasSibling = (query) => sibilings.has(query);
          if (isFileSystemDirectoryHandle(handle2) && !isFolderExcluded(path, basename3, hasSibling)) {
            dirs.push(processDirectory(handle2, path + "/", ignoreFile));
          } else if (isFileSystemFileHandle(handle2) && isFileIncluded(path, basename3, hasSibling)) {
...
function pathIncludedInQuery(queryProps, path, extUri2) {
  ...
  if (queryProps.includePattern || queryProps.usingSearchPaths) {
    ...
    if (queryProps.usingSearchPaths) {
      return !!queryProps.folderQueries && queryProps.folderQueries.some((fq) => {
        ...
        if (extUri2.isEqualOrParent(uri, searchPath)) {
