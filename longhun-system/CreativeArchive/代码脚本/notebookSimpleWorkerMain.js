...
// out-build/vs/workbench/contrib/notebook/common/services/notebookSimpleWorker.js
...
var CellSequence = class _CellSequence {
  ...
  static async createWithCellId(textModel) {
    const hashValue = /* @__PURE__ */ new Map();
    await Promise.all(textModel.cells.map(async (c, i) => {
      const value = await c.getComparisonValue();
      const id2 = c.metadata?.id || "";
      hashValue.set(id2, value);
    }));
    return hashValue;
  }
...
var NotebookEditorSimpleWorker = class {
  constructor() {
...
// out-build/vs/workbench/contrib/notebook/common/services/notebookSimpleWorkerMain.js
