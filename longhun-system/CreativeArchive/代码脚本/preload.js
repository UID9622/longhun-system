...
(function () {
    ...
    // Expose welcome API for the welcome setup pages
    // Only expose if we are on a welcome page to avoid unwanted IPC communications
    const isWelcomePage = window.location.href.includes('/vs/code/electron-main/welcome/');
    if (isWelcomePage) {
        try {
            const welcomeAPI = Object.freeze({
                navigate: (page) => ipcRenderer.send('welcome:navigate', page),
                complete: () => ipcRenderer.invoke('welcome:complete'),
                importConfig: () => ipcRenderer.invoke('welcome:importConfig'),
                login: () => ipcRenderer.invoke('welcome:login'),
                onShowImportProgress: (callback) => {
                    ipcRenderer.on('showImportProgress', (event, data) => {
                        callback(data);
                    });
                },
                minimizeWindow: () => ipcRenderer.send('welcome:window:minimize'),
                maximizeWindow: () => ipcRenderer.send('welcome:window:maximize'),
                closeWindow: () => ipcRenderer.send('welcome:window:close')
            });
            contextBridge.exposeInMainWorld('welcomeAPI', welcomeAPI);
        }
        catch (error) {
            console.error('Failed to expose welcomeAPI:', error);
        }
    }
}());
