...
var LanguageDetectionWorkerHost = class _LanguageDetectionWorkerHost {
  static {
    this.CHANNEL_NAME = "languageDetectionWorkerHost";
  }
  static getChannel(workerServer) {
    return workerServer.getChannel(_LanguageDetectionWorkerHost.CHANNEL_NAME);
  }
  static setChannel(workerClient, obj) {
    workerClient.setChannel(_LanguageDetectionWorkerHost.CHANNEL_NAME, obj);
  }
};
...
// out-build/vs/workbench/services/languageDetection/browser/languageDetectionSimpleWorker.js
function create(workerServer) {
  return new LanguageDetectionSimpleWorker(workerServer);
}
var LanguageDetectionSimpleWorker = class _LanguageDetectionSimpleWorker {
  static {
  ...
  async $detectLanguage(uri, langBiases, preferHistory, supportedLangs) {
    ...
    const neuralResolver = async () => {
      for await (const language2 of this.detectLanguagesImpl(documentTextSample)) {
        if (!this.modelIdToCoreId.has(language2.languageId)) {
          this.modelIdToCoreId.set(language2.languageId, await this._host.$getLanguageId(language2.languageId));
        }
        const coreId = this.modelIdToCoreId.get(language2.languageId);
        if (coreId && (!supportedLangs?.length || supportedLangs.includes(coreId))) {
          languages.push(coreId);
          confidences.push(language2.confidence);
        }
      }
      stopWatch.stop();
      if (languages.length) {
  ...
  adjustLanguageConfidence(modelResult) {
    switch (modelResult.languageId) {
      ...
      case "xml":
      case "php":
        modelResult.confidence += _LanguageDetectionSimpleWorker.positiveConfidenceCorrectionBucket1;
        break;
      case "cpp":
      case "sh":
      case "java":
      case "cs":
      case "c":
        modelResult.confidence += _LanguageDetectionSimpleWorker.positiveConfidenceCorrectionBucket2;
        break;
      case "bat":
      case "ini":
      case "makefile":
      ...
      case "csv":
      case "toml":
        modelResult.confidence -= _LanguageDetectionSimpleWorker.negativeConfidenceCorrection;
        break;
      default:
        break;
    }
  ...
  async *detectLanguagesImpl(content) {
    ...
    if (!modelResults || modelResults.length === 0 || modelResults[0].confidence < _LanguageDetectionSimpleWorker.expectedRelativeConfidence) {
      return;
    }
    const firstModelResult = this.adjustLanguageConfidence(modelResults[0]);
    if (firstModelResult.confidence < _LanguageDetectionSimpleWorker.expectedRelativeConfidence) {
      return;
    }
    ...
    for (let current of modelResults) {
      ...
      if (currentHighest.confidence - current.confidence >= _LanguageDetectionSimpleWorker.expectedRelativeConfidence) {
        while (possibleLanguages.length) {
          yield possibleLanguages.shift();
        }
        if (current.confidence > _LanguageDetectionSimpleWorker.expectedRelativeConfidence) {
...
// out-build/vs/workbench/services/languageDetection/browser/languageDetectionSimpleWorkerMain.js
