...
var SubProductName, Scenario, CoreMode, ProductFamilyName;
...
var LANGUAGE_DEFAULT, _isWindows, _isMacintosh, _isLinux, _isLinuxSnap, _isNative, _isWeb, _isElectron, _isIOS, _isCI, _isMobile, _locale, _language, _platformLocale, _translationsConfigFile, _userAgent, $globalThis, nodeProcess, isElectronProcess, isElectronRenderer, Platform, _platform, isWindows, isMacintosh, isLinux, isNative, isWeb, isWebWorker, webWorkerOrigin, userAgent, language, Language, setTimeout0IsFaster, setTimeout0, OperatingSystem, isChrome, isFirefox, isSafari, isEdge, isAndroid;
...
var safeProcess, INVALID_UID, INVALID_GID, vscodeGlobal, cwd, env, platform, arch, getuid, getgid;
var init_process = __esm({
  "out-build/vs/base/common/process.js"() {
    ...
    getuid = safeProcess.getuid;
    getgid = safeProcess.getgid;
  }
});

// out-build/vs/platform/product/common/product.js
var product2, vscodeGlobal2, RAW_PACKAGE_ORGANIZATION, product_default;
var init_product2 = __esm({
  "out-build/vs/platform/product/common/product.js"() {
    ...
    product2.isVSCode = product2.subProductName === SubProductName.VSCodeHuawei;
    RAW_PACKAGE_ORGANIZATION = "basic";
    product2.isMinSystem = product2.isVSCode && product2.organization !== RAW_PACKAGE_ORGANIZATION;
  ...
  "out-build/vs/base/common/event.js"() {
    ...
    (function(Event2) {
      ...
      function toPromise(event) {
        return new Promise((resolve4) => once(event)(resolve4));
      }
...
var CHAR_UPPERCASE_A, CHAR_LOWERCASE_A, CHAR_UPPERCASE_Z, CHAR_LOWERCASE_Z, CHAR_DOT, CHAR_FORWARD_SLASH, CHAR_BACKWARD_SLASH, CHAR_COLON, CHAR_QUESTION_MARK, ErrorInvalidArgType, platformIsWin32, win32, posixCwd, posix, normalize, isAbsolute, join3, resolve2, relative, dirname3, basename, extname, format, parse, toNamespacedPath, sep, delimiter;
var init_path = __esm({
  "out-build/vs/base/common/path.js"() {
    "use strict";
    init_process();
    ...
    posix = {
      ...
      resolve(...pathSegments) {
        ...
        for (let i = pathSegments.length - 1; i >= -1 && !resolvedAbsolute; i--) {
          const path4 = i >= 0 ? pathSegments[i] : posixCwd();
          validateString(path4, `paths[${i}]`);
          if (path4.length === 0) {
            continue;
          }
...
function hasDriveLetter(path4, isWindowsOS = isWindows) {
  if (isWindowsOS) {
...
function getServerRootPath(product3, basePath) {
  return posix.join(basePath ?? "/", `${product3.quality ?? "oss"}-${product3.commit ?? "dev"}`);
}
  ...
  "out-build/vs/base/common/network.js"() {
    ...
    RemoteAuthoritiesImpl = class {
      ...
      setServerRootPath(product3, serverBasePath) {
        this._serverRootPath = getServerRootPath(product3, serverBasePath);
      }
      getServerRootPath() {
...
var ExtUri, extUri, extUriBiasedIgnorePathCase, extUriIgnorePathCase, isEqual, isEqualOrParent2, getComparisonKey, basenameOrAuthority, basename2, extname2, dirname4, joinPath, normalizePath, relativePath, resolvePath, isAbsolutePath, isEqualAuthority, hasTrailingPathSeparator, removeTrailingPathSeparator, addTrailingPathSeparator, DataUri;
...
function writeFile2(path4, data, options) {
  return writeQueues.queueFor(URI.file(path4), () => {
    ...
    return new Promise((resolve4, reject) => doWriteFileAndFlush(path4, data, ensuredOptions, (error) => error ? reject(error) : resolve4()));
...
async function realcase(path4, token) {
  if (isLinux) {
    return path4;
  }
  ...
  const name = (basename(path4) || path4).toLowerCase();
  try {
...
var StringIterator, ConfigKeysIterator, PathIterator, UriIteratorState, UriIterator, TernarySearchTreeNode, Dir, TernarySearchTree;
    ...
    NodeJSFileWatcherLibrary = class _NodeJSFileWatcherLibrary extends Disposable {
      ...
      async watch() {
        try {
          ...
          const stat = await promises5.stat(realPath);
          if (this.cts.token.isCancellationRequested) {
            return;
          }
      ...
      for (const deprecatedId of o.deprecates) {
        if (remainingArgs.hasOwnProperty(deprecatedId)) {
          if (!val) {
            ...
            if (val) {
              errorReporter.onDeprecatedOption(deprecatedId, o.deprecationMessage || localize(1828, null, optionId));
            }
          }
...
function buildHelpMessage(productName, executableName, version, options, capabilities) {
  ...
  const inputFiles = capabilities?.noInputFiles !== true ? `[${localize(1829, null)}...]` : "";
  const help = [`${productName} ${version}`];
  help.push("");
  help.push(`${localize(1830, null)}: ${executableName} [${localize(1831, null)}]${inputFiles}`);
  help.push("");
  if (capabilities?.noPipe !== true) {
    if (isWindows) {
      help.push(localize(1832, null, executableName));
    } else {
      help.push(localize(1833, null, executableName));
    }
    help.push("");
  }
...
function buildVersionMessage(version, commit) {
  return `${version || localize(1835, null)}
${commit || localize(1836, null)}
${process.arch}`;
}
...
var init_argv = __esm({
  "out-build/vs/platform/environment/node/argv.js"() {
    ...
    init_product2();
    helpCategories = {
      o: localize(1788, null),
      e: localize(1789, null),
      t: localize(1790, null)
    };
    NATIVE_CLI_COMMANDS = ["tunnel", "serve-web"];
    OPTIONS = {
      "tunnel": {
        type: "subcommand",
        description: `Make the current machine accessible from ${product_default.isVSCode ? "vscode.dev" : "codearts.dev"} or other machines through a secure tunnel`,
        options: {
          "cli-data-dir": { type: "string", args: "dir", description: localize(1791, null) },
      ...
      "serve-web": {
        ...
        options: {
          "cli-data-dir": { type: "string", args: "dir", description: localize(1792, null) },
          "disable-telemetry": { type: "boolean" },
          "telemetry-level": { type: "string" }
        }
      },
      "diff": { type: "boolean", cat: "o", alias: "d", args: ["file", "file"], description: localize(1793, null) },
      "merge": { type: "boolean", cat: "o", alias: "m", args: ["path1", "path2", "base", "result"], description: localize(1794, null) },
      "add": { type: "boolean", cat: "o", alias: "a", args: "folder", description: localize(1795, null) },
      "goto": { type: "boolean", cat: "o", alias: "g", args: "file:line[:character]", description: localize(1796, null) },
      "new-window": { type: "boolean", cat: "o", alias: "n", description: localize(1797, null) },
      "reuse-window": { type: "boolean", cat: "o", alias: "r", description: localize(1798, null) },
      "wait": { type: "boolean", cat: "o", alias: "w", description: localize(1799, null) },
      "waitMarkerFilePath": { type: "string" },
      "locale": { type: "string", cat: "o", args: "locale", description: localize(1800, null) },
      "user-data-dir": { type: "string", cat: "o", args: "dir", description: localize(1801, null) },
      "profile": { type: "string", "cat": "o", args: "profileName", description: localize(1802, null) },
      "help": { type: "boolean", cat: "o", alias: "h", description: localize(1803, null) },
      "extensions-dir": { type: "string", deprecates: ["extensionHomePath"], cat: "e", args: "dir", description: localize(1804, null) },
      "extensions-download-dir": { type: "string" },
      "builtin-extensions-dir": { type: "string" },
      "list-extensions": { type: "boolean", cat: "e", description: localize(1805, null) },
      "show-versions": { type: "boolean", cat: "e", description: localize(1806, null) },
      "category": { type: "string", allowEmptyValue: true, cat: "e", description: localize(1807, null), args: "category" },
      "install-extension": { type: "string[]", cat: "e", args: "ext-id | path", description: localize(1808, null) },
      "pre-release": { type: "boolean", cat: "e", description: localize(1809, null) },
      "uninstall-extension": { type: "string[]", cat: "e", args: "ext-id", description: localize(1810, null) },
      "update-extensions": { type: "boolean", cat: "e", description: localize(1811, null) },
      "enable-proposed-api": { type: "string[]", allowEmptyValue: true, cat: "e", args: "ext-id", description: localize(1812, null) },
      "version": { type: "boolean", cat: "t", alias: "v", description: localize(1813, null) },
      "verbose": { type: "boolean", cat: "t", global: true, description: localize(1814, null) },
      "log": { type: "string[]", cat: "t", args: "level", global: true, description: localize(1815, null) },
      "status": { type: "boolean", alias: "s", cat: "t", description: localize(1816, null) },
      "prof-startup": { type: "boolean", cat: "t", description: localize(1817, null) },
      ...
      "disable-extensions": { type: "boolean", deprecates: ["disableExtensions"], cat: "t", description: localize(1818, null) },
      "disable-extension": { type: "string[]", cat: "t", args: "ext-id", description: localize(1819, null) },
      "sync": { type: "string", cat: "t", description: localize(1820, null), args: ["on | off"] },
      "inspect-extensions": { type: "string", allowEmptyValue: true, deprecates: ["debugPluginHost"], args: "port", cat: "t", description: localize(1821, null) },
      "inspect-brk-extensions": { type: "string", allowEmptyValue: true, deprecates: ["debugBrkPluginHost"], args: "port", cat: "t", description: localize(1822, null) },
      "disable-lcd-text": { type: "boolean", cat: "t", description: localize(1823, null) },
      "disable-gpu": { type: "boolean", cat: "t", description: localize(1824, null) },
      "disable-chromium-sandbox": { type: "boolean", cat: "t", description: localize(1825, null) },
      "sandbox": { type: "boolean" },
      "telemetry": { type: "boolean", cat: "t", description: localize(1826, null, product_default.familyName) },
      ...
      "dev-mode": { type: "boolean", deprecates: ["devMode"], description: localize(1827, null) },
      // chromium flags
...
function parseAndValidate(cmdLineArgs, reportWarnings) {
  const onMultipleValues = (id2, val) => {
    console.warn(localize(1837, null, id2, val));
  };
  const onEmptyValue = (id2) => {
    console.warn(localize(1838, null, id2));
  };
  const onDeprecatedOption = (deprecatedOption, message) => {
    console.warn(localize(1839, null, deprecatedOption, message));
  };
  const getSubcommandReporter = (command) => ({
    onUnknownOption: (id2) => {
      if (!NATIVE_CLI_COMMANDS.includes(command)) {
        console.warn(localize(1840, null, id2, command));
      }
    },
    onMultipleValues,
    onEmptyValue,
  ...
  const errorReporter = {
    onUnknownOption: (id2) => {
      console.warn(localize(1841, null, id2));
    },
    onMultipleValues,
    onEmptyValue,
    onDeprecatedOption,
  ...
  if (args.goto) {
    args._.forEach((arg) => assert(/^(\w:)?[^:]+(:\d*){0,2}:?$/.test(arg), localize(1842, null)));
  }
  return args;
}
...
async function resolveTerminalEncoding(verbose) {
  ...
  if (cliEncodingEnv) {
    ...
    rawEncodingPromise = new Promise((resolve4) => {
      if (verbose) {
        ...
        if (stdout) {
          ...
          for (const key of windowsTerminalEncodingKeys) {
            if (stdout.indexOf(key) >= 0) {
              return resolve4(windowsTerminalEncodings[key]);
            }
          }
        }
    ...
    rawEncodingPromise = new Promise((resolve4) => {
      ...
      exec("locale charmap", (err, stdout, stderr) => resolve4(stdout));
    });
  }
...
function stdinDataListener(durationinMs) {
  return new Promise((resolve4) => {
    const dataListener = () => resolve4(true);
    setTimeout(() => {
...
async function readFromStdin(targetPath, verbose, onEnd) {
  let [encoding, iconv] = await Promise.all([
    ...
    fs6.promises.appendFile(targetPath, "")
    ...
    if (args[subcommand]) {
      ...
      return new Promise((resolve4, reject) => {
        ...
        if (process.env["VSCODE_DEV"]) {
          ...
          const tunnelCommand = join3(appPath, "bin", `${product_default.tunnelApplicationName}${isWindows ? ".exe" : ""}`);
          tunnelProcess = spawn(tunnelCommand, [subcommand, ...tunnelArgs], { cwd: cwd(), stdio, env: env2 });
        }
        tunnelProcess.stdout.pipe(process.stdout);
        tunnelProcess.stderr.pipe(process.stderr);
        tunnelProcess.on("exit", resolve4);
        tunnelProcess.on("error", reject);
      });
    }
  }
  if (args.help) {
    const executable = `${product_default.applicationName}${isWindows ? ".exe" : ""}`;
    ...
    if (args.wait) {
      ...
      processCallbacks.push(async (child2) => {
        ...
        if (isMacOSBigSurOrNewer) {
          childExitPromise = new Promise((resolve4) => {
    ...
    if (!isMacOSBigSurOrNewer) {
      ...
      if (env2["VSCODE_DEV"]) {
        ...
        if (launchDirIndex !== -1) {
          spawnArgs[launchDirIndex] = resolve2(curdir);
        }
      }
...
if (productObj["BUILD_INSERT_PRODUCT_CONFIGURATION"]) {
  ...
  productObj.isVSCode = productObj.subProductName === SubProductName.VSCodeHuawei;
}
...
async function resolveNLSConfiguration({ userLocale, osLocale, userDataPath, commit, nlsMetadataPath, appRootPath }) {
  ...
  try {
    const languagePacks = await getLanguagePackConfigurations(userDataPath, appRootPath, userLocale);
    if (!languagePacks) {
      return defaultNLSConfiguration(userLocale, osLocale, nlsMetadataPath);
    }
...
async function getLanguagePackConfigurations(userDataPath, appRootPath, locale) {
  if (product_default.isVSCode) {
    ...
    let configDataInUserDataPath;
    try {
      const configFile = path3.join(userDataPath, "languagepacks.json");
      configDataInUserDataPath = JSON.parse(await fs3.promises.readFile(configFile, "utf-8"));
    } catch (err) {
      return await handleLanguagePacksEmptyScene(appRootPath, locale);
    }
    const resolvedLocale = resolveLanguagePackLanguage(configDataInUserDataPath, locale);
    if (!configDataInUserDataPath || !resolvedLocale) {
      return await handleLanguagePacksEmptyScene(appRootPath, locale);
    }
    return configDataInUserDataPath;
  }
}
async function handleLanguagePacksEmptyScene(appRootPath, locale) {
  try {
    const languagePacksFilePathInAppRoot = path3.join(appRootPath, "languagepacks.json");
    const isDefaultLanguagePacksFilePathExist = await exists(languagePacksFilePathInAppRoot);
    if (!isDefaultLanguagePacksFilePathExist) {
      return void 0;
    }
    const defaultConfigData = await fs3.promises.readFile(languagePacksFilePathInAppRoot, "utf-8");
    const pathSplitPatten = /\\/g;
    const formattedAppRootPath = appRootPath.replace(pathSplitPatten, "/");
    const appRootPatten = /{{APPROOT}}/g;
    const defaultConfigDataReplacedRootPath = defaultConfigData.replace(appRootPatten, formattedAppRootPath);
    const formattedJsonConfigData = JSON.parse(defaultConfigDataReplacedRootPath);
    const resolvedLocale = resolveLanguagePackLanguage(formattedJsonConfigData, locale);
    if (!resolvedLocale) {
      return void 0;
    }
    const packageJsonFile = path3.resolve(formattedJsonConfigData[resolvedLocale].translations.vscode, "../../package.json");
    const packageJsonData = JSON.parse(await fs3.promises.readFile(packageJsonFile, "utf-8"));
    const languagePackVersion = packageJsonData.version;
    formattedJsonConfigData[resolvedLocale].extensions[0].version = languagePackVersion;
    const extensionId = formattedJsonConfigData[resolvedLocale].extensions[0].extensionIdentifier.id;
    const hash = generateLanguagePackHash(languagePackVersion, extensionId);
    formattedJsonConfigData[resolvedLocale].hash = hash;
    return formattedJsonConfigData;
  } catch (e) {
  }
  return void 0;
}
function generateLanguagePackHash(languagePackVersion, extensionId) {
  const md5 = createHash("md5");
  md5.update(extensionId).update(languagePackVersion);
...
var nlsConfiguration = await resolveNLSConfiguration({ userLocale: "en", osLocale: "en", commit: product.commit, userDataPath: "", nlsMetadataPath: __dirname3, appRootPath: resolve3(__dirname3, "../") });
