...
var Promises;
(function(Promises3) {
  async function settled(promises5) {
    let firstError = void 0;
    const result = await Promise.all(promises5.map((promise) => promise.then((value) => value, (error) => {
      if (!firstError) {
        firstError = error;
      }
      return void 0;
...
(function(ProxyChannel2) {
  function fromService(service2, disposables, options) {
...
var ParcelWatcher = class _ParcelWatcher extends BaseWatcher {
  ...
  normalizePath(request) {
    ...
    try {
      realPath = realpathSync2(request.path);
    ...
    for (const requestsForCorrelation of mapCorrelationtoRequests.values()) {
      ...
      for (const request of requestsForCorrelation.values()) {
        if (requestTrie.findSubstr(request.path)) {
          try {
            const realpath2 = realpathSync2(request.path);
            if (realpath2 === request.path) {
...
var NodeJSFileWatcherLibrary = class _NodeJSFileWatcherLibrary extends Disposable {
  ...
  async watch() {
    try {
      ...
      const stat = await promises4.stat(realPath);
      if (this.cts.token.isCancellationRequested) {
        return;
      }
