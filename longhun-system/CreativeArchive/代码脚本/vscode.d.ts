...
declare module 'vscode' {
	...
	export class CodeLens {
		...
		/**
		 * The font style of this code lens.
		 */
		fontStyle?: CodeLensFontStyle;
		 ...
		 * @param fontStyle The font style of this code lens.
		 */
		constructor(range: Range, command?: Command, fontStyle?: CodeLensFontStyle);
	}
	...
	export class CompletionItem {
		...
		/**
				 * A string or snippet that should be inserted in a document when selecting
				 * this completion in replace mode. Fallbacks to {@link CompletionItem.insertText insertText} if `falsy`.
				 */
		replaceText?: string | SnippetString;

		/**
		 * An optional array of additional text edits that are applied when
		 * selecting this completion in replace mode. Edits must not overlap with the main edit
		 * nor with themselves. Fallbacks to {@link CompletionItem.additionalTextEdits additionalTextEdits} if `falsy`.
		 */
		additionalReplaceEdits?: TextEdit[];
	...
	export interface CompletionItemProvider<T extends CompletionItem = CompletionItem> {
		...
		/**
		 * Provide conditional breakpoints completion items for the given position and document.
		 *
		 * @param document The document in which the command was invoked.
		 * @param position The position at which the command was invoked.
		 * @param token A cancellation token.
		 * @param context How the completion was triggered.
		 * @param positionInInputBox The position at which the command was invoked in conditional breakpoint input box.
		 * @param expressionText Expression in the condition breakpoint input box
		 *
		 * @return An array of completions, a {@link CompletionList completion list}, or a thenable that resolves to either.
		 * The lack of a result can be signaled by returning `undefined`, `null`, or an empty array.
		 */
		provideBreakpointCompletionItems?(document: TextDocument, position: Position, token: CancellationToken, context: CompletionContext, positionInInputBox: Position, expressionText: string): ProviderResult<T[] | CompletionList<T>>;
	...
	export class Diagnostic {
		...
		/**
		 * Property to classify diagnostics by custom category in the same file, diagnostics
		 * with the same category will be sorted to the same group.
		 */
		category?: string;
	...
	export interface AuthenticationProvider {
		 ...
		 */
		getSessions(scopes: readonly string[] | undefined, options: AuthenticationProviderSessionOptions, isBuiltInExtension?: boolean, extensionId?: string): Thenable<AuthenticationSession[]>;
