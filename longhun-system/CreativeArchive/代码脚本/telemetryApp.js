...
(function(win323) {
  async function findExecutable(command, cwd2, paths) {
    ...
    for (const pathEntry of paths) {
      ...
      if (isAbsolute(pathEntry)) {
        ...
        fullPath = join(cwd2, pathEntry, command);
      }
  ...
  win323.findExecutable = findExecutable;
})(win322 || (win322 = {}));
...
var ChannelServer = class {
  ...
  registerChannel(channelName, channel2) {
    this.channels.set(channelName, channel2);
  ...
  onPromise(request2) {
    const channel2 = this.channels.get(request2.channelName);
    if (!channel2) {
      this.collectPendingRequest(request2);
      return;
    }
    const cancellationTokenSource = new CancellationTokenSource();
    let promise;
    try {
      promise = channel2.call(this.ctx, request2.name, request2.arg, cancellationTokenSource.token);
    } catch (err) {
    ...
    promise.then((data) => {
      ...
      disposable.dispose();
      this.activeRequests.delete(request2.id);
    });
    const disposable = toDisposable(() => cancellationTokenSource.cancel());
    this.activeRequests.set(request2.id, disposable);
  }
  onEventListen(request2) {
    const channel2 = this.channels.get(request2.channelName);
    if (!channel2) {
      this.collectPendingRequest(request2);
      return;
    ...
    const id2 = request2.id;
    const event = channel2.listen(this.ctx, request2.name, request2.arg);
  ...
  disposeActiveRequest(request2) {
    const disposable = this.activeRequests.get(request2.id);
    if (disposable) {
      disposable.dispose();
      this.activeRequests.delete(request2.id);
    }
  }
  collectPendingRequest(request2) {
    let pendingRequests = this.pendingRequests.get(request2.channelName);
    if (!pendingRequests) {
      pendingRequests = [];
      this.pendingRequests.set(request2.channelName, pendingRequests);
    }
    const timer = setTimeout(() => {
      ...
      if (request2.type === 100) {
        this.sendResponse({
          id: request2.id,
          data: { name: "Unknown channel", message: `Channel name '${request2.channelName}' timed out after ${this.timeoutDelay}ms`, stack: void 0 },
          type: 202
          /* ResponseType.PromiseError */
        });
      }
    ...
    if (requests) {
      for (const request2 of requests) {
        ...
        switch (request2.request.type) {
          case 100:
            this.onPromise(request2.request);
            break;
          case 102:
            this.onEventListen(request2.request);
            break;
        }
      }
...
(function(ProxyChannel2) {
  ...
  function toService(channel2, options) {
    const disableMarshalling = options && options.disableMarshalling;
    return new Proxy({}, {
      get(_target, propKey) {
        if (typeof propKey === "string") {
          ...
          if (propertyIsDynamicEvent(propKey)) {
            return function(arg) {
              return channel2.listen(propKey, arg);
            };
          }
          if (propertyIsEvent(propKey)) {
            return channel2.listen(propKey);
          }
          return async function(...args) {
            ...
            const result = await channel2.call(propKey, methodArgs);
...
var AMDModuleImporter = class _AMDModuleImporter {
  ...
  async _nodeJSLoadScript(scriptSrc) {
    try {
      ...
      const content = fs2.readFileSync(filePath).toString();
...
// out-build/vs/workbench/contrib/debug/node/telemetryApp.js
var appender = new OneDataSystemAppender(void 0, false, process.argv[2], JSON.parse(process.argv[3]), process.argv[4]);
process.once("exit", () => appender.flush());
var channel = new TelemetryAppenderChannel([appender]);
var server = new Server("telemetry");
