...
// 修正 grpc-tools 的路径，使其指向插件工程根目录
const GRPC_TOOLS_PROTO_PATH = path.resolve("../node_modules/grpc-tools/bin")

const isWindows = process.platform === "win32"
const TS_PROTO_PLUGIN = path.resolve("../node_modules/.bin/protoc-gen-ts_proto.cmd") // Use the .bin directory path for Windows
...
async function compileProtos() {
	...
	// Add node_modules/grpc-tools/bin to proto_path for descriptor set generation as well
	const descriptorProtocCommand = [
		PROTOC,
		`--proto_path="${GRPC_TOOLS_PROTO_PATH}"`, // Add grpc-tools proto path first
		`--proto_path="${PROTO_DIR}"`,          // Then project's proto path
...
async function tsProtoc(outDir, protoFiles, protoOptions) {
	...
	// Add node_modules/grpc-tools/bin to proto_path to find google/protobuf/*.proto files
	const command = [
		PROTOC,
		`--proto_path="${GRPC_TOOLS_PROTO_PATH}"`, // Add grpc-tools proto path first
		`--proto_path="${PROTO_DIR}"`,          // Then project's proto path
