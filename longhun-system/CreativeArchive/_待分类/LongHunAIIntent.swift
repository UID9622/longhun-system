...
1. 把这个文件添加到LongHunWidget项目
2. 在Xcode里Build一次
3. 对Siri说:
   - "Hey Siri，问龍魂" → 与本地AI对话
   - "Hey Siri，搜索龍魂记忆" → 搜索本地记忆
   - "Hey Siri，保存到龍魂" → 保存信息
   - "Hey Siri，查询龍魂知识库" → 搜索Notion
   - "Hey Siri，启动三色审计" → L2审计
   - "Hey Siri，生成DNA追溯码" → 生成DNA
   - "Hey Siri，查询龍魂状态" → 查询系统
