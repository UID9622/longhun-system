...
def get(url: str, headers: dict | None = None, timeout: float = 15.0) -> dict:
...
def post(url: str, data=None, json=None, headers: dict | None = None, timeout: float = 15.0) -> dict:
    ...
    req = _ur.Request(url, data=body, headers={**base_headers, **(headers or {})}, method="POST")
    try:
        with _opener(timeout).open(req, timeout=timeout) as r:
            return {"status": r.status, "headers": dict(r.headers),
                    "body": r.read().decode("utf-8", "replace"), "error": None}
